...
class Inventory():
    ...
    def addSlotShellsData(self, aircraftID, weaponSlotID, count, storedCount):
        self.aircraftsData[aircraftID][ISYNC_AIRCRAFT.PDATA_AIRCRAFT_DICT][PLANE_KEYS.SHELLS][weaponSlotID] = {'count': int(count),
         'storedCount': storedCount}

    def getSlotShellsData(self, aircraftID, weaponSlotID):
        planeData = self.aircraftsData[aircraftID][ISYNC_AIRCRAFT.PDATA_AIRCRAFT_DICT]
        return planeData[PLANE_KEYS.SHELLS].setdefault(weaponSlotID, {'count': 0})

    def __getBeltsData(self, aircraftID):
        return self.aircraftsData[aircraftID][ISYNC_AIRCRAFT.PDATA_AIRCRAFT_DICT][PLANE_KEYS.BELTS]

    def __setBeltsData(self, aircraftID, data):
        self.aircraftsData[aircraftID][ISYNC_AIRCRAFT.PDATA_AIRCRAFT_DICT][PLANE_KEYS.BELTS] = data

    def __setShellsData(self, aircraftID, data):
        self.aircraftsData[aircraftID][ISYNC_AIRCRAFT.PDATA_AIRCRAFT_DICT][PLANE_KEYS.SHELLS] = data

    def __getShellsData(self, planeID):
        return self.aircraftsData[planeID][ISYNC_AIRCRAFT.PDATA_AIRCRAFT_DICT][PLANE_KEYS.SHELLS]
    ...
    def setPlaneBelt(self, aircraftID, weaponSlotID, beltID, count):
        ...
        @param count:
        """
        self.aircraftsData[aircraftID][ISYNC_AIRCRAFT.PDATA_AIRCRAFT_DICT][PLANE_KEYS.BELTS][weaponSlotID] = [beltID, count]
    ...
    def getPlaneBeltID(self, aircraftID, weaponSlotID):
        ...
        @return:
        """
        belt = self.aircraftsData[aircraftID][ISYNC_AIRCRAFT.PDATA_AIRCRAFT_DICT][PLANE_KEYS.BELTS].get(weaponSlotID, None)
        if belt is None:
            return
        else:
            return belt[0]
    ...
    def __getPlaneBeltData(self, aircraftID, weaponSlotID):
        ...
        @return:
        """
        return self.aircraftsData[aircraftID][ISYNC_AIRCRAFT.PDATA_AIRCRAFT_DICT][PLANE_KEYS.BELTS].get(weaponSlotID, None)
    ...
    def __setInstalledGlobalID(self, aircraftID, globalID):
        self.aircraftsData[aircraftID][ISYNC_AIRCRAFT.PDATA_AIRCRAFT_DICT][PLANE_KEYS.INSTALLED_GLOBAL_ID] = globalID

    def __getInstalledConfiguration(self, aircraftID):
        globalID = self.aircraftsData[aircraftID][ISYNC_AIRCRAFT.PDATA_AIRCRAFT_DICT][PLANE_KEYS.INSTALLED_GLOBAL_ID]
    ...
    def _setAicraftsData(self, args):
        for resp_arr in args:
            for resp in resp_arr:
                ...
                self.aircraftsData[planeID] = resp
    ...
    def getInstalledUpgradesGlobalID(self, planeID):
        ...
        """
        if planeID:
            return self.aircraftsData[planeID][ISYNC_AIRCRAFT.PDATA_AIRCRAFT_DICT][PLANE_KEYS.INSTALLED_GLOBAL_ID]
        else:
            return None
    ...
    def getLastGameTime(self, airPlaneID, default = -1):
        ...
        if airPlaneID == -1:
            lgt = -1
            for k in self.aircraftsData.keys():
                tmp = self.getLastGameTime(k)
                if tmp > lgt:
                    lgt = tmp

            return lgt
        pd = self.aircraftsData.get(airPlaneID, None)
        if pd is None:
            return default
    ...
    def isUpgradeBought(self, upgName):
        if self.getUpgradeCount(upgName) > 0:
            return True
        for aircraft in self.aircraftsData.itervalues():
    ...
    def getInstalledCamouflages(self, aircraftID):
        return self.aircraftsData[aircraftID][ISYNC_AIRCRAFT.CAMOUFLAGES_DICT]
    ...
    def getCamouflageNationDecalID(self, camouflageID):
        ...
        @rtype: int
        """
        camouflage = self._db.getCamouflage(camouflageID)
        if not camouflage:
    ...
    def dailyFirstWinXPFactor(self, aircraftID):
        return self.aircraftsData[aircraftID][ISYNC_AIRCRAFT.DAILY_FIRST_WIN_XP_FACTOR]

    def dailyFirstWinRemains(self, aircraftID):
        return self.aircraftsData[aircraftID][ISYNC_AIRCRAFT.DAILY_FIRST_WIN_REMAINS]

    def getAvailableUpgrades(self):
        return self.inventoryData[ISYNC_INVENTORY.DEPOT_UPGRADES_DICT]

    def getAircraftSlotIndexById(self, aircraftID):
        l = self.inventoryData[ISYNC_INVENTORY.BOUGHT_AIRCRAFTS_LIST]
        if aircraftID in l:
    ...
    def getBoughtAircraftsList(self):
        ...
        @rtype: list
        """
        return self.inventoryData[ISYNC_INVENTORY.BOUGHT_AIRCRAFTS_LIST]

    def openedAircrafts(self):
        return self.inventoryData[ISYNC_INVENTORY.OPENED_AIRCRAFTS_LIST]

    def inBattlePlaneList(self):
        return self.inventoryData[ISYNC_INVENTORY.BATTLE_AIRCRAFTS_LIST]

    def getUnlockPlanes(self):
        return self.inventoryData[ISYNC_INVENTORY.UNLOCK_AIRCRAFTS]

    def addPlaneToUnlocks(self, aircraftID):
        self.inventoryData[ISYNC_INVENTORY.UNLOCK_AIRCRAFTS][aircraftID] = None
        if aircraftID in self.inBattlePlaneList():
            self.inventoryData[ISYNC_INVENTORY.BATTLE_AIRCRAFTS_LIST].remove(aircraftID)
        return

    def removePlaneFromUnlocks(self, aircraftID):
        del self.inventoryData[ISYNC_INVENTORY.UNLOCK_AIRCRAFTS][aircraftID]
    ...
    def getAircraftsDataByNationID(self, nationID):
        ...
        for aircraftID in aircraftMap.keys():
            ...
            if not self._db.isPlaneExclusive(aircraftID) and not self._db.isPlaneNPC(aircraftID) or self._db.forceShowInNationTree(aircraftID):
    ...
    def getCarouselAircrafts(self):
        airplaneList = []
        for aircraftID in self.aircraftsData:
            if self.aircraftsData[aircraftID][ISYNC_AIRCRAFT.SLOT_INDEX] is None:
                continue
    ...
    def addAircrafttoBoughtList(self, aircraftID):
        self.inventoryData[ISYNC_INVENTORY.BOUGHT_AIRCRAFTS_LIST].append(aircraftID)

    def removeAircraftFromBoughtList(self, aircraftID):
        del self.inventoryData[ISYNC_INVENTORY.BOUGHT_AIRCRAFTS_LIST][self.getAircraftSlotIndexById(aircraftID)]
    ...
    def getDepotWeapons(self):
        ...
        """
        return dict(((upgName, upgCount) for upgName, upgCount in self.inventoryData[ISYNC_INVENTORY.DEPOT_UPGRADES_DICT].iteritems() if getattr(self._db.upgrades.get(upgName, None), 'type', '') in UPGRADE_TYPE.WEAPON))
    ...
    def getDepotEquipment(self):
        ...
        """
        return self.inventoryData[ISYNC_INVENTORY.DEPOT_EQUIPMENT_DICT]
    ...
    def getDepotConsumables(self):
        ...
        """
        return self.inventoryData[ISYNC_INVENTORY.DEPOT_CONSUMABLES_DICT]
    ...
    def filterPlanesByLevel(self, lowerLevelBound, upperLevelBound):
        """Iterate through bought planes with level in [lower; upper] interval
        @param lowerLevelBound: Lower plane level bound
        @param upperLevelBound: Upper plane level bound
        @return: Iterator through plane IDs
        @rtype: __generator[int]
        """
        for planeID in self.getBoughtAircraftsList():
            settings = db.DBLogic.g_instance.getAircraftData(planeID)
            if lowerLevelBound <= settings.airplane.level <= upperLevelBound:
                yield planeID

    def filterUnlockedPlanesByLevel(self, lowerLevelBound, upperLevelBound):
        """Iterate through bought planes with level in [lower; upper] interval that are ready for battle
        @param lowerLevelBound: Lower plane level bound
        @param upperLevelBound: Upper plane level bound
        @return: Iterator through plane IDs
        @rtype: __generator[int]
        """
        for planeID in self.filterPlanesByLevel(lowerLevelBound, upperLevelBound):
            blockType = self.aircraftsData[planeID][ISYNC_AIRCRAFT.PDATA_AIRCRAFT_DICT][PLANE_KEYS.BLOCK_TYPE]
            if blockType == BLOCK_TYPE.UNLOCKED:
                yield planeID
...
class InventoryClientBase(Inventory):
    ...
    def setDailyFirstWinRemains(self, aircraftID, val):
        self.aircraftsData[aircraftID][ISYNC_AIRCRAFT.DAILY_FIRST_WIN_REMAINS] = val

    def setDailyFirstWinXPCoeff(self, aircraftID, val):
        self.aircraftsData[aircraftID][ISYNC_AIRCRAFT.DAILY_FIRST_WIN_XP_FACTOR] = val
