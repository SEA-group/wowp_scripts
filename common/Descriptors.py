...
import Math
__teamObjectMask = '=LH?Hhhhh'
__teamObjectUpdateMask = '=L?h'

def createTeamObjectCompactDescriptor(entityID, arenaObjID, isAlive, maxHealth, teamIndex, position):
    return struct.pack(__teamObjectMask, entityID, arenaObjID, isAlive, maxHealth, teamIndex, int(position.x + 0.5), int(position.y + 0.5), int(position.z + 0.5))


def createTeamObjectCompactUpdateDescriptor(entityID, isAlive, teamIndex):
    return struct.pack(__teamObjectUpdateMask, entityID, isAlive, teamIndex)
...
def getTeamObjectDescriptorFromCompactDescriptor(compactDescriptor):
    ...
    return {'id': data[0],
     ...
     'maxHealth': data[3],
     'teamIndex': data[4],
     'pos': Math.Vector3(data[5], data[6], data[7])}


def getTeamObjectDescriptorFromCompactUpdateDescriptor(compactDescriptor):
    data = struct.unpack(__teamObjectUpdateMask, compactDescriptor)
    return {'id': data[0],
     'isAlive': data[1],
     'teamIndex': data[2]}
