...
def removeFromCache(ids, types, iface, callbacks, subscriable = False):
    ...
    if not subscriable:
        debug_observable.IfaceDebugOutput(COMMAND_TYPES.UNSUBSCRIBE, ifacename=iface, obtype=types, obid=ids, callbacks=callbacks)
...
def addToCache(ids, types, iface, callbacks, subscriable = False):
    ...
    if not subscriable:
        ...
        debug_observable.IfaceDebugOutput(COMMAND_TYPES.SUBSCRIBE, ifacename=iface, obtype=types, obid=ids, callbacks=callbacks)
