...
class EntitySupportedClasses:
    ...
    def isAvatarClassID(classID):
        return classID in (EntitySupportedClasses.Avatar, EntitySupportedClasses.AvatarBot, EntitySupportedClasses.Bomber)
                ...
                if weaponType:
                    for weapon in weaponType.weapons:
                        ...
                        if weaponSlotFMData:
                            ...
                            if hasattr(item, 'recoilDispersion'):
                                weapon.recoilDispersion = item.recoilDispersion
                            if hasattr(item, 'autoguiderAngle'):
                                weapon.autoguiderAngle = item.autoguiderAngle
                            if hasattr(item, 'overheatingFullTime'):
                                weapon.overheatingFullTime = item.overheatingFullTime
                            if hasattr(item, 'gunRestartTime'):
                                weapon.gunRestartTime = item.gunRestartTime
                            if hasattr(item, 'coolingCFC'):
                                weapon.coolingCFC = item.coolingCFC
                            if hasattr(item, 'respawnTempo'):
                                weapon.respawnTempo = item.respawnTempo
                            if hasattr(item, 'portion'):
                                weapon.portion = item.portion
                            if hasattr(item, 'portionTempo'):
                                weapon.portionTempo = item.portionTempo
                        yield weapon
...
def getAirplaneWeaponSettings(airplaneSettings):
    from copy import deepcopy
    classSettings = deepcopy(db.DBLogic.g_instance.getAircraftClassDescription(airplaneSettings.planeType).weaponOptions)
    weaponSettings = airplaneSettings.flightModel.weaponOptions
    for k, v in weaponSettings.__dict__.iteritems():
        setattr(classSettings, k, v)
...
def canAimToEnemyEntity(owner, entity):
...
class TurretsStateHandler(object):
    ...
    def entityUnderTurretAttack(self, entityID):
        for turretID, state in self.__data.iteritems():
            if entityID == state.targetID and state.isFire:
                return turretID

        return -1

    def getMaxAggroTargetID(self):
        if len(self.__data):
            return sorted(self.__data.itervalues(), key=lambda el: el.normAggro)[-1].targetID
        return -1

    def get(self, key):
        if key not in self.__data:
            self.__data[key] = TurretsStateHandler.element()
...
def getBombGravityAcceleration(planeType = -1):
    gravityAcceleration = db.DBLogic.g_instance.aircrafts.environmentConstants.gravityAcceleration
    return WORLD_SCALING * BOMB_ACCELERATION_CFC.get(planeType, 1) * gravityAcceleration


def getAirResistance(planeType = -1):
    return AIR_RESISTANCE * AIR_RESISTANCE_CFC.get(planeType, 1)