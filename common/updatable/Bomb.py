...
import db.DBLogic
from EntityHelpers import EntityStates, getBombGravityAcceleration, getAirResistance
...
class Bomb(UpdatableObjectBase):
    SEND_SIGNATURE_STRING = '16sfffffffffffII'
    BACKUP_SIGNATURE_STRING = 'fffffffffff'
    CLIENT_COLLISION_EXPANDING = 5.0

    def __init__(self, owner):
        UpdatableObjectBase.__init__(self, owner)
        self.accelerationV = Math.Vector3(0, -1, 0)
        self.airResistance = 0
        self.__initPhysicParameters(owner.settings.airplane.planeType)
        self.model = None
        self.__enemyList = []
        self.matrix = None
    ...
    def __initPhysicParameters(self, planeType):
        self.accelerationV = getBombGravityAcceleration(planeType)
        self.airResistance = getAirResistance(planeType)
    ...
    def setState(self, state, timeShift = 0.0):
        ...
        if IS_CLIENT and self._owner:
            ...
            if curState == UPDATABLE_STATE.DESTROY:
                ...
                rotation = Math.Vector3(uniform(0.0, radians(360.0)), 0.0, 0.0)
                EffectManager.g_instance.createWorldEffect(Effects.getEffectId(explosionParticle), self.position, {'rotation': rotation})
    ...
    def __calcPos(self, t):
        addPosition = (self._startVector + self.__dispersionAccel * t) * (1.0 - math.exp(-self.airResistance * t)) / self.airResistance
    ...
    def _onCreate(self, args):
        ...
        self.__initPhysicParameters(args[13])
        Q = Math.Quaternion()
        Q.fromAngleAxis(self._startVector.yaw, Math.Vector3(0, 1, 0))
        self.__dispersionAccel = Q.rotateVec(self.__dispersionAccel)
        self.__shellDescription = db.DBLogic.g_instance.getComponentByIndex(COMPONENT_TYPE.BOMBS, self._resourceID)
    ...
    def __onRocketModelLoaded(self, resourceRefs):
        if self._owner is not None:
            ...
            if modelName not in resourceRefs.failedIDs:
                ...
                self.__setModel()
                if IS_CLIENT:
                    self.__updatableSound.startSound(getattr(self.__shellDescription, 'weaponSoundID', None), self.position, self.getState())
            else:
    ...
    def getCreationArgs(self):
        ...
        sendList += self._startPosition.list() + self._startVector.list() + self._startRotation.list() + [self.__dispersionAccel.x,
         self.__dispersionAccel.z,
         self._owner.id,
