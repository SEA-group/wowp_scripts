...
class Turret(DBBaseClass):
    ...
    def __init__(self, typeID, fileName, data):
        ...
        if data:
            ...
            if IS_EDITOR:
                self.hangarNameNormal = self.hangarName
    ...
    def readData(self, data):
        if data:
            ...
            readValue(self, data, 'nation', '')
            readValue(self, data, 'parent', '')
    ...
    def save(self):
        if IS_AIRPLANE_EDITOR:
            shellsSection = findSection(self.__data, 'shells', missingCheck=False)
            if shellsSection is not None:
                shellPath = findSection(shellsSection, 'shellPath', missingCheck=False)
                while shellPath is not None:
                    shellsSection.deleteSection('shellPath')
                    shellPath = findSection(shellsSection, 'shellPath', missingCheck=False)

                shellsSection.writeStrings('shellPath', self.shellPathes)
            flamesSection = findSection(self.__data, 'flames', missingCheck=False)
            if flamesSection is not None:
                flamePath = findSection(flamesSection, 'flamePath', missingCheck=False)
                while flamePath is not None:
                    flamesSection.deleteSection('flamePath')
                    flamePath = findSection(flamesSection, 'flamePath', missingCheck=False)

                flamesSection.writeStrings('flamePath', self.flamePathes)
            self.targetLockShootDistance /= WORLD_SCALING
            writeValue(self, self.__data, 'targetLockShootDistance', 0.0)
            self.targetLockShootDistance *= WORLD_SCALING
            self.targetLostDistance /= WORLD_SCALING
            writeValue(self, self.__data, 'targetLostDistance', 1000.0)
            self.targetLostDistance *= WORLD_SCALING
            writeValue(self, self.__data, 'name', '')
            writeValue(self, self.__data, 'parent', '')
            writeValue(self, self.__data, 'weaponCount', 1)
            writeValue(self, self.__data, 'critAbility', 1.0)
            writeValue(self, self.__data, 'serverRPS', 1.0)
            writeValue(self, self.__data, 'burstDelay', 1.0)
            writeValue(self, self.__data, 'burstTime', 1.0)
            writeValue(self, self.__data, 'nation', '')
            writeValue(self, self.__data, 'DPS', 0.0)
            self.__data.writeString('hangarName', self.hangarNameNormal)
            self.__data.writeString('gunName', self.gunName)
            ikSect = findSection(self.__data, 'ik', missingCheck=False)
            if ikSect is None:
                ikSect = self.__data.createSection('ik')
            ikSect.writeString('model', self.modelIk)
            try:
