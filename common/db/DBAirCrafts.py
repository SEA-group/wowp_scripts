...
class AirCraft():
    ...
    def __init__(self, dataPath, newDatabase):
        ...
        if IS_EDITOR:
            self.__data = data
        if IS_EDITOR or IS_DEVELOPMENT:
    ...
    def setMagnetPoints(self, points):
        self.airplane.magnetPoints = points
...
class AirCraftTunes():
    ...
    def calculateMass(self, installedLogicalParts = None, installedUpgrades = None):
        """
        Calculates aircraft's mass
        @param installedLogicalParts: parts that will be used in calculation or None for default parts
        @param installedUpgrades: upgrade names, see airplanes configurations db
        """
        if installedLogicalParts is None:
            installedLogicalParts = [0] * LOGICAL_PART.COUNT
        if installedUpgrades is None:
            installedUpgrades = []
        hull = self.flightModel.hull[installedLogicalParts[LOGICAL_PART.HULL]]
        engine = self.flightModel.engine[installedLogicalParts[LOGICAL_PART.ENGINE]]
        mass = hull.mass + engine.mass
        for s in self.flightModel.turretSlot:
            for t in s.turret:
                if t in installedUpgrades:
                    mass += t.mass
