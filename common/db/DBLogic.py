...
from consts import AIRCRAFTS_PATH, BASES_PATH, IS_CLIENT, IS_EDITOR, IS_CELLAPP, IS_BASEAPP, ARENA_TYPE_PATH, TURRETS_PATH, EFFECTS_PATH, MATERIAL_NAMES_PATH, SPLINES_PATH, CURVES_PATH, BATTLE_ALERTS_PATH, BATTLE_NOTIFICATION_PATH, GUI_TEXTURES_PATH, LOGICAL_PART, CAMERA_MISC_PATH, UPGRADE_TYPE, LOGICAL_PART_STRING_TO_INT_TYPE, AIRPLANE_CAMERA_PRESETS_PATH, TEAM_OBJECT_CLASS_NAMES, AVATAR_CLASS_NAMES, BLOCK_TYPE, BATTLE_DURATIONS, TIME_OUT_AFTER_BATTLE, COMPONENT_TYPE, calculateGunDPS, PLANE_KEYS, CREW_MEMBER, CAMERAEFFECTS_PATH, HANGAR_PATH, SOUND_SETTINGS_PATH, USER_HANGAR_SPACES_PATH, ARENA_TYPE, SPECS_KEY_MAP, LANDSCAPE_MATERIALS_PATH, FRACTION, AC_GAME_MODE_PRESETS_PATH, PLANE_TYPE, FAST_COMMANDS_SETTINGS_PATH, HUD_PLANE_SCHEME_PATH
...
from DBHUDHints import BattleAlerts, BattleNotifications
from BDHUDPlaneScheme import PlaneScheme
...
import os
from HelperFunctions import findIf, select, reflectObject
...
import _warAction
from FastCommandsConfiguration import FastCommandModel
from db.DBSakingPartsProfiles import DBShakingPartsProfiles
from DBAreaConquest.Hud.ACHudHint import ACHudHint
from DBAreaConquest.Hud.ACHudSettings import ACHudSettings
from DBAreaConquest.Hud.ACRadarSettings import ACRadarSettings
from BigWorld import Spline
from BigWorld import Curve2D
try:
...
def initDB(onlyEntities = False, loadFromCache = False, skipEntities = False):
    ...
    if g_instance == None:
        ...
        if loadFromCache:
            LOG_TRACE('load DB from cache')
            g_instance = tryUnpicleDB()
        if g_instance is None:
            LOG_TRACE('DB cache in not ready. Create DB')
...
class NotExistingUpgradeVariant(Exception):
    ...
    def __init__(self, db, aircraftID, upgradeName):
        from pprint import pformat
        aircraftData = db.getAircraftData(aircraftID)
        upgrades = db.upgrades[upgradeName]
        message = 'Cant find proper variant in installed upgrade'
        message += '\n\n======================= aircraft data ======================\n\n'
        message += pformat(reflectObject(aircraftData))
        message += '\n\n======================= upgrade data =======================\n\n'
        message += pformat(reflectObject(upgrades))
        super(NotExistingUpgradeVariant, self).__init__(message)
...
class db():
    ...
    @property
    def camouflages(self):
        """
        Returns upgrades as map where key - upgrade name, value - upgrade
        """
        return self.__camouflagesDatabase
    ...
    def getHangarSettings(self):
        if self.__soundSettings is None:
            return
        else:
            return self.__soundSettings.hangar
    ...
    def getMapSoundSettings(self):
        if self.__soundSettings is None:
            return
        else:
            return self.__soundSettings.musicSound.events
    ...
    def __loadSplines(self):
        self.__splines = dict()
        folderPath = SPLINES_PATH
        listPath = '%s/list.xml' % folderPath
        ...
        if listData is not None:
            ...
            for fileName in names:
                ...
                if fileData is not None:
                    for splineID, splineData in fileData['mapActivities'].items():
                        spline_name_lower = splineID.lower()
                        spline = Spline(spline_name_lower, splineData)
                        if IS_EDITOR:
                            spline.setGroupName(fileName)
                            spline.setDefaultGroupName(fileName)
                        self.__splines[spline_name_lower] = spline
    ...
    def __loadCurves(self):
        self.__curves = dict()
        folderData = ResMgr.openSection(CURVES_PATH)
        if folderData is not None:
            for curve in folderData.values():
                curveName = curve.readString('name')
                LOG_INFO("Loading Curve '{0}'".format(curveName))
                self.__curves[curveName] = Curve2D(curve)

        ResMgr.purge(CURVES_PATH, True)
        return

    def reloadCurves(self):
        if IS_DEVELOPMENT:
            self.__loadCurves()

    def reloadSplines(self):
        if IS_DEVELOPMENT:
            self.__loadSplines()

    def getCurve(self, curveName):
        lowerName = curveName.lower()
        curve = self.__curves.get(lowerName, None)
        if not curve and curveName:
            LOG_ERROR("can't find curve", lowerName)
        return curve
    ...
    def __init__(self, onlyEntities = False, skipEntities = False):
        ...
        if onlyEntities:
            ...
            loadingProgressStep('initFastComands')
            self.__fastCommandsConfig = FastCommandModel()
            if IS_CLIENT:
                ...
                self.readAirplaneCameraPresets()
                loadingProgressStep('readAirplaneCameraPresets')
                battleAlertHintData = ResMgr.openSection(BATTLE_ALERTS_PATH)
                if battleAlertHintData:
                    self.__battleAlertsHints = BattleAlerts(battleAlertHintData)
                    ResMgr.purge(BATTLE_ALERTS_PATH, True)
                print ('BATTLE_ALERTS_PATH', BATTLE_ALERTS_PATH, battleAlertHintData)
                loadingProgressStep('BATTLE_ALERTS_PATH')
                battleNotHintData = ResMgr.openSection(BATTLE_NOTIFICATION_PATH)
                if battleNotHintData:
                    self.__battleNotificationHints = BattleNotifications(battleNotHintData)
                    ResMgr.purge(BATTLE_NOTIFICATION_PATH, True)
                print ('BATTLE_NOT_PATH', BATTLE_NOTIFICATION_PATH, battleNotHintData)
                loadingProgressStep('BATTLE_NOT_PATH')
                self.__hudPlaneSchemes, _ = self.__readDataListFromFolder(HUD_PLANE_SCHEME_PATH, PlaneScheme)
                self.__acHudHints = ACHudHint()
                self.__acHudSettings = ACHudSettings()
                self.__acRadarSettings = ACRadarSettings()
                self.readCameraEffects()
            ...
            self.__shakingPartsProfiles = None
            self.readShakingPartsProfiles()
            return
    ...
    def __initCommon(self):
        ...
        self.__battleAlertsHints = None
        self.__battleNotificationHints = None
        self.__hudPlaneSchemes = None
        self.__acHudHints = None
        self.__acHudSettings = None
        self.__acRadarSettings = None
        return

    def readShakingPartsProfiles(self):
        self.__shakingPartsProfiles = DBShakingPartsProfiles()

    @property
    def shakingPartsProfiles(self):
        return self.__shakingPartsProfiles
    ...
    def getBattleAlertHints(self):
        return self.__battleAlertsHints.getAllHints()

    def getBattleAlertHintByID(self, ID):
        return self.__battleAlertsHints.getHint(ID)

    def getBattleNotificationHints(self):
        return self.__battleNotificationHints.getAllHints()

    def getBattleNotificationHintByID(self, ID):
        return self.__battleNotificationHints.getHint(ID)

    def getHudPlaneSchemeByName(self, name):
        return self.__hudPlaneSchemes.get(name)
    ...
    def getSlotType(self, aircraftId, weaponSlotConfigurationId):
        slotType = ''
        aircraftData = self.getAircraftData(aircraftId)
        slots = aircraftData.components.weapons2.slots
        for slot in slots:
            wType = slots[slot].types.get(weaponSlotConfigurationId, None)
            if wType:
                weapons = wType.weapons
                if weapons:
                    slotType = self.upgrades[weapons[0].name].type
    ...
    def __initEntities(self):
        ...
        for arenaID, arenaData in self.__entities[DBEntities.ARENAS][1].iteritems():
            ...
            if IS_BASEAPP or IS_CELLAPP:
                from _arenasload import LoadByMapID
                mapLoad = LoadByMapID.get(arenaID)
                if mapLoad is not None:
                    maxPlayers = 30
                    if mapLoad.players[-1].count < maxPlayers:
                        LOG_ERROR('Missing load values for maximum players number=%s for mapID=%s in _arenasload' % (maxPlayers, arenaID))
                else:
                    LOG_ERROR('No load value for mapID=%s found in _arenasload' % arenaID)
    ...
    def __readDataWithFixedIDsFromFolder(self, folderPath, dataClass):
        ...
        for objSection in listData.values():
            if not objSection.has_key('name'):
                continue
            if not (IS_DEVELOPMENT or IS_EDITOR) and objSection.has_key('build') and objSection.readString('build') == 'DEVELOPMENT':
                continue
    ...
    def isArenasValid(self, ids):
        return all([ self.isArenaValid(arena) for arena in ids ])
        ...
        for upgName in upgradeNames:
            ...
            if uprade.type not in UPGRADE_TYPE.WEAPON:
                ...
                if upgradeVariant is None:
                    raise NotExistingUpgradeVariant(self, aircraftID, upgName)
    ...
    def getLogicalPartIndex(self, logicalPartType, logicalPartName, aircraftId, default = 0):
        ...
        @type logicalPartName: string
        @type aircraftId: int
        @type default: int
        @return: logical part index or default value
        """
        logicalParts = None
    ...
    def forceShowInNationTree(self, aircraftID):
        return getattr(self.getAircraftData(aircraftID).airplane.options, 'forceViewInTree', False)

    def isPlaneBotAcceptable(self, planeID):
        return getattr(self.getAircraftData(planeID).airplane.options, 'isBotAcceptable', True)

    def isBotRocketeerPlane(self, planeID):
        return getattr(self.getAircraftData(planeID).airplane.options, 'isBotRocketeer', True)
    ...
    def getAllUpgradeVariants(self, aircraftID):
        from db.DBLogicHelpers.TurretsHelpers import findTurretSlotByTurretName
        aircraftData = self.getAircraftData(aircraftID)
        aircraftName = aircraftData.airplane.name
        upgradeModules = self.getAircraftUpgrades(aircraftID)[0]
        upgradeByTypes = {}
        for upgradeModule in filter(lambda upgradeModule: upgradeModule.type in UPGRADE_TYPE.MODULES, upgradeModules):
            key = upgradeModule.type
            if upgradeModule.type == UPGRADE_TYPE.TURRET:
                turretSlot = findTurretSlotByTurretName(aircraftData, upgradeModule.name)
                if turretSlot is None:
                    continue
                key = '{}_{}'.format(upgradeModule.type, turretSlot.ID)
            upgradeByTypes[key] = upgradeByTypes.get(key, []) + [upgradeModule]
    ...
    def findAircraftPartIDsByUpgradeName(self, aircraftName, upgradeName):
        upgrade = self.upgrades[upgradeName]
        for variant in upgrade.variant:
            if variant.aircraftName != aircraftName:
                continue
            for part in variant.part:
                yield part.id

            break

    def findUpgradeNamesByGlobalIDAndType(self, globalID, upgradeType):
        upgradeNames = airplanesConfigurations[globalID].modules
    ...
    def getAircraftDataByGlobalID(self, globalID):
        return self.getAircraftData(airplanesConfigurations[globalID].planeID)
    ...
    def readAirplaneCameraPresets(self):
    ...
    def hasGunner(self, aircraftId):
        aircraftData = self.getAircraftData(aircraftId)
        for partSettings in aircraftData.airplane.partsSettings.getPartsOnlyList():
    ...
    def getPlaneMinMaxSpec(self, planeID, tag):
        specKey = SPECS_KEY_MAP[tag]
        variant = _performanceCharacteristics_db.airplaneCharacteristicsBoundaries.get(planeID, None)
        if variant is not None:
            return (variant[0].__dict__[specKey], variant[1].__dict__[specKey])
        else:
            return (0, 0)
    ...
    def getShootingDistanceEffective(self, globalID):
        ...
        maxShootingDistance = self.getShootingDistanceMax(globalID)
        planeData = self.getAircraftData(planeID)
        return maxShootingDistance * planeData.airplane.flightModel.weaponOptions.bulletMinFlightDist

    def getShootingDistanceMax(self, globalID):
        """
        Get max shooting distance for specified plane configuration.
        @param globalID: Configuration, to process.
        @type globalID: int
        @return: Max shooting distance
        @rtype: float
        """
        configuration = getAirplaneConfiguration(globalID)
        planeID, weaponSlots = configuration.planeID, configuration.weaponSlots
        maxShootingDistance = 0.0
    ...
    def getACGameModePreset(self, name):
        """Return Area Conquest game mode settings preset by name
        @param name: Settings preset name
        @rtype: db.DBAreaConquest.ArenaGameModeSettings.ArenaGameModeSettings
        """
        return self.__acGameModePresets[name]

    def getACHudHints(self):
        """
        @rtype: db.DBAreaConquest.Hud.ACHudHint
        """
        return self.__acHudHints

    def getACHudSettings(self):
        """
        @rtype: db.DBAreaConquest.Hud.ACHudSettings
        """
        return self.__acHudSettings

    def getACRadarSettings(self):
        """
        @rtype: db.DBAreaConquest.Hud.ACRadarSettings
        """
        return self.__acRadarSettings

    def getFastCommands(self):
        return self.__fastCommandsConfig.configuration

    def __initCamouflages(self):
        import _camouflages_data
        self.__camouflagesDatabase = _camouflages_data.CamouflagesDB
        self.__camouflagesIDs = _camouflages_data.CamouflagesIDs
        self.__camouflagesFilters = _camouflages_data.Filter

    def getCamouflage(self, globalCamouflageID):
        return self.__camouflagesDatabase.get(globalCamouflageID, None)

    def getCamouflages(self, **kw):
        res = self.__camouflagesIDs
        for key, value in kw.iteritems():
            res = res.intersection(self.__camouflagesFilters.get(key, dict()).get(value, set()))
