...
class ExternalModifiers(AvatarControllerBase):
    ...
    def __init__(self, owner, consumables, equipment, crewSkills):
        ...
        readyToUseSubscriptions = (EventSubscription(c.eReadyToUse, self._onConsumableReadyToUse) for c in self.__objects[OBJ_GROUPS.CONSUMABLES])
        self._subscription = CompositeSubscription(*readyToUseSubscriptions)
        if IS_CELLAPP:
            self._subscription.extend(EventSubscription(self._owner.eDestruction, self.onOwnerDeath))
    ...
    def onControllersCreated(self):
        ...
        for slotID, slotData in enumerate(self._owner.consumables):
            if slotData['key'] != -1:
                consumable = db.DBLogic.g_instance.getConsumableByID(slotData['key'])
    ...
    def onOwnerRespawn(self, *args, **kwargs):
        ...
        needUpdate = any([ consumable.onOwnerRespawn(currentTime) for consumable in self.consumables ])
        if needUpdate:
            self.reCalc()
