# Embedded file name: scripts/common/GameEventsCommon/template/builder.py
...
class BlockBuilder(object):
    ...
    def add(self, lineno, code):
        if lineno < self.lineno:
            raise SyntaxError('Inconsistence at %s : %s' % (self.lineno, lineno))
        if lineno == self.lineno:
            line = self.buf[-1]
            if code:
                if line:
    ...
    def build_token(self, lineno, token, value):
        if token in self.rules:
            ...
            raise SyntaxError('No rule to build "%s" token at line %d.' % (token, lineno))
...
class SourceBuilder(object):
    ...
    def build_render(self, nodes):
        ...
        builder.add(self.lineno + 1, 'def render(ctx, local_defs, super_defs):')
        builder.start_block()
