# Embedded file name: scripts/common/GameEventsCommon/template/preprocessor.py
...
class Preprocessor(object):
    ...
    def __init__(self, runtime_engine_factory, engine, key_factory):
        ...
        self.engine.template_class = lambda name, render_template: template_class(name, lambda ctx, local_defs, super_defs: self.render(name, ctx, local_defs, super_defs))
    ...
    def ensure_runtime_engine(self, key):
        ...
        try:
            ...
            if key in engines:
                return engines[key]
            engine = engines[key] = self.runtime_engine_factory(loader=ChainLoader([DictLoader({}), self.engine.loader]))
