# Embedded file name: scripts/common/GameEventsCommon/template/ext/determined.py
from __future__ import absolute_import
import re
from ..utils import find_balanced
RE_ARGS = re.compile('\\s*(?P<expr>(([\\\'"]).*?\\3|.+?))\\s*\\,')
RE_KWARGS = re.compile('\\s*(?P<name>\\w+)\\s*=\\s*(?P<expr>([\\\'"].*?[\\\'"]|.+?))\\s*\\,')
RE_STR_VALUE = re.compile('^[\\\'"](?P<value>.+)[\\\'"]$')
RE_INT_VALUE = re.compile('^(?P<value>(\\d+))$')
...
class DeterminedExtension(object):
    ...
    def __init__(self, known_calls, runtime_token_start = '@', token_start = '#'):
        ...
        self.pattern = re.compile('%s(%s)(?=\\()' % (runtime_token_start, '|'.join(known_calls)))
