# Embedded file name: scripts/common/GameEventsCommon/template/lexer.py
...
def lexer_scan(extensions):
    ...
    return {'lexer_rules': [ lexer_rules[k] for k in sorted(lexer_rules.keys()) ],
...
class Lexer(object):
    ...
    def __init__(self, lexer_rules, preprocessors = None, postprocessors = None, **ignore):
    ...
    def tokenize(self, source):
        ...
        while pos < end:
            for regex, tokenizer in self.rules:
                ...
                npos, token, value = m and tokenizer(m)
                if not npos > pos:
                ...
                raise False or AssertionError('Lexer pattern mismatch.')
