# Embedded file name: scripts/common/GameEventsCommon/validation/schema/utils.py
...
def compare_paths_lt(x, y):
    for i in range(min(len(x), len(y))):
        if isinstance(x[i], type(y[i])):
            ...
            if isinstance(x[i], _int_types):
                return True
            if isinstance(y[i], _int_types):
                return False
...
def validator_factory(name, mixin = None, class_dict = {}):
    """ Dynamically create a :class:`~.Validator` subclass.
