# Embedded file name: scripts/common/GameEventsCommon/validation/schema/schema.py
...
class DefinitionSchema(MutableMapping):
    ...
    def __init__(self, validator, schema = {}):
        ...
        self.schema_validator = SchemaValidator(None, allow_unknown=self.validation_schema, error_handler=errors.SchemaErrorHandler, target_schema=schema, target_validator=validator)
    ...
    def _expand_logical_shortcuts(cls, schema):
        ...
        def is_of_rule(x):
            return isinstance(x, _str_type) and x.startswith(('allof_', 'anyof_', 'noneof_', 'oneof_'))
    ...
    def _expand_subschemas(cls, schema):
        ...
        def has_mapping_schema():
            ...
            aimed to mappings. """
            try:
                return all((isinstance(x, Mapping) for x in schema[field]['schema'].values()))
            except TypeError:
                return False
    ...
    def _validate(self, schema):
        ...
        :param schema: The schema to be validated as a legal . schema
...
class SchemaValidatorMixin(object):
    """ This validator is extended to validate schemas passed to a .
    validator. """
    ...
    def _validate_logical(self, rule, field, value):
        ...
        validator = self._get_child_validator(document_crumb=rule, schema=self.root_allow_unknown['schema'], allow_unknown=self.root_allow_unknown['allow_unknown'])
    ...
    def _validator_bulk_schema(self, field, value):
        ...
        if _hash in self.target_validator._valid_schemas:
            ...
            validator = self._get_child_validator(document_crumb=field, schema=self.root_allow_unknown['schema'], allow_unknown=self.root_allow_unknown['allow_unknown'])
    ...
    def _validator_handler(self, field, value):
        if isinstance(value, Callable):
            return
        if isinstance(value, _str_type):
            if value not in self.target_validator.validators + self.target_validator.coercers:
    ...
    def _validator_schema(self, field, value):
        ...
        if _hash in self.target_validator._valid_schemas:
            ...
            validator = self._get_child_validator(document_crumb=field, schema=None, allow_unknown=self.root_allow_unknown)
            validator(value, normalize=False)
            if validator._errors:
                self._error(validator._errors)
            else:
