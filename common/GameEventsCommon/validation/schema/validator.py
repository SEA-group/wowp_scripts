# Embedded file name: scripts/common/GameEventsCommon/validation/schema/validator.py
"""
    Extensible validation for Python dictionaries.
    This module implements . Validator class
...
from .schema import schema_registry, rules_set_registry, DefinitionSchema, SchemaError
...
class Validator(object):
    ...
    or upon calling the :meth:`~.Validator.validate`,
    :meth:`~.Validator.validated` or
    :meth:`~.Validator.normalized` method. An instance itself is
    ...
    :param error_handler: The error handler that formats the result of
                          ...
                          Default: :class:`~.errors.BasicErrorHandler`.
    :type error_handler: class or instance based on
                         :class:`~.errors.BaseErrorHandler` or
                         :class:`tuple`
    """
    ...
    def __init_error_handler(self, kwargs):
        ...
        if isinstance(error_handler, tuple):
            ...
            eh_config = {}
        if isclass(error_handler) and issubclass(error_handler, errors.BaseErrorHandler):
            return error_handler(**eh_config)
        if isinstance(error_handler, errors.BaseErrorHandler):
    ...
    def _error(self, *args):
        ...
        :param args: Accepts different argument's signatures.
                     ...
                     *2. Custom error:*
                     ...
                     added to :attr:`~.Validator._errors`.
                     ...
                     *3. Defined error:*
                     ...
                     A :class:`~.errors.ValidationError` instance will
        ...
        if len(args) == 1:
            ...
            self.recent_error = errors.ValidationError(document_path, schema_path, code, rule, constraint, value, info)
    ...
    def _drop_nodes_from_errorpaths(self, _errors, dp_items, sp_items):
        ...
        for error in _errors:
            for i in sorted(dp_items, reverse=True):
                error.document_path = drop_item_from_tuple(error.document_path, dp_basedepth + i)

            for i in sorted(sp_items, reverse=True):
                error.schema_path = drop_item_from_tuple(error.schema_path, sp_basedepth + i)
    ...
    def errors(self):
        ...
        bound to :attr:`~.Validator.error_handler`. """
    ...
    def is_child(self):
        ...
        :meth:`~.Validator._get_child_validator`.
        Type: :class:`bool` """
    ...
    def root_allow_unknown(self):
        """ The :attr:`~.Validator.allow_unknown` attribute of the
    ...
    def root_document(self):
        """ The :attr:`~.Validator.document` attribute of the
        first level ancestor of a child validator. """
    ...
    def root_schema(self):
        """ The :attr:`~.Validator.schema` attribute of the
        first level ancestor of a child validator. """
    ...
    def _normalize_coerce(self, mapping, schema):
        ...
        for field in mapping:
            if field in schema and 'coerce' in schema[field]:
                mapping[field] = self.__normalize_coerce(schema[field]['coerce'], field, mapping[field], schema[field].get('nullable', False), error)
            elif isinstance(self.allow_unknown, Mapping) and 'coerce' in self.allow_unknown:
                mapping[field] = self.__normalize_coerce(self.allow_unknown['coerce'], field, mapping[field], self.allow_unknown.get('nullable', False), error)
    ...
    def __normalize_coerce(self, processor, field, value, nullable, error):
        if isinstance(processor, _str_type):
            ...
            for p in processor:
                ...
                if errors.COERCION_FAILED in self.document_error_tree.fetch_errors_from(self.document_path + (field,)):
                    break
        ...
        for field in mapping:
            ...
            if isinstance(mapping[field], Mapping):
                if 'keyschema' in schema[field]:
                    self.__normalize_mapping_per_keyschema(field, mapping, schema[field]['keyschema'])
                if 'valueschema' in schema[field]:
                    self.__normalize_mapping_per_valueschema(field, mapping, schema[field]['valueschema'])
                if set(schema[field]) & set(('allow_unknown', 'purge_unknown', 'schema')):
                    try:
    ...
    def __normalize_mapping_per_keyschema(self, field, mapping, property_rules):
        ...
        validator = self._get_child_validator(document_crumb=field, schema_crumb=(field, 'keyschema'), schema=schema)
        ...
        for k in result:
            ...
            if result[k] in mapping[field]:
                warn('Normalizing keys of {path}: {key} already exists, its value is replaced.'.format(path='.'.join(self.document_path + (field,)), key=k))
    ...
    def __normalize_mapping_per_valueschema(self, field, mapping, value_rules):
        ...
        validator = self._get_child_validator(document_crumb=field, schema_crumb=(field, 'valueschema'), schema=schema)
        mapping[field] = validator.normalized(mapping[field], always_return_document=True)
        if validator._errors:
            self._drop_nodes_from_errorpaths(validator._errors, [], [2])
            self._error(validator._errors)

    def __normalize_mapping_per_schema(self, field, mapping, schema):
        validator = self._get_child_validator(document_crumb=field, schema_crumb=(field, 'schema'), schema=schema[field].get('schema', {}), allow_unknown=schema[field].get('allow_unknown', self.allow_unknown), purge_unknown=schema[field].get('purge_unknown', self.purge_unknown))
    ...
    def __normalize_sequence(self, field, mapping, schema):
        schema = dict(((k, schema[field]['schema']) for k in range(len(mapping[field]))))
        document = dict(((k, v) for k, v in enumerate(mapping[field])))
        validator = self._get_child_validator(document_crumb=field, schema_crumb=(field, 'schema'), schema=schema)
        value_type = type(mapping[field])
        result = validator.normalized(document, always_return_document=True)
    ...
    def __normalize_rename_fields(self, mapping, schema):
        for field in tuple(mapping):
            ...
            elif isinstance(self.allow_unknown, Mapping) and 'rename_handler' in self.allow_unknown:
                self._normalize_rename_handler(mapping, {field: self.allow_unknown}, field)
    ...
    def _normalize_rename_handler(self, mapping, schema, field):
        ...
        if 'rename_handler' not in schema[field]:
            return
        new_name = self.__normalize_coerce(schema[field]['rename_handler'], field, field, False, errors.RENAMING_FAILED)
        if new_name != field:
            mapping[new_name] = mapping[field]
            del mapping[field]

    def __validate_readonly_fields(self, mapping, schema):
        for field in (x for x in schema if x in mapping and self._resolve_rules_set(schema[x]).get('readonly')):
            self._validate_readonly(schema[field]['readonly'], field, mapping[field])

    def __normalize_default_fields(self, mapping, schema):
        fields = [ x for x in schema if x not in mapping or mapping[x] is None and not schema[x].get('nullable', False) ]
        try:
        ...
        while fields:
            ...
            if fields_state in known_fields_states:
                for field in fields:
                    self._error(field, errors.SETTING_DEFAULT_FAILED, 'Circular dependencies of default setters.')
    ...
    def validated(self, *args, **kwargs):
        """ Wrapper around :meth:`~.Validator.validate` that returns
    ...
    def __validate_unknown_fields(self, field):
        if self.allow_unknown:
            ...
            if isinstance(self.allow_unknown, (Mapping, _str_type)):
                ...
                validator = self._get_child_validator(schema_crumb=schema_crumb, schema={field: self.allow_unknown})
    ...
    def __validate_definitions(self, definitions, field):
        ...
        value = self.document[field]
        rules_queue = [ x for x in self.priority_validations if x in definitions or x in self.mandatory_validations ]
        rules_queue.extend((x for x in self.mandatory_validations if x not in rules_queue))
        rules_queue.extend((x for x in definitions if x not in rules_queue and x not in self.normalization_rules and x not in ('allow_unknown', 'required')))
    ...
    _validate_allow_unknown = dummy_for_rule_validation(" {'oneof': [{'type': 'boolean'},\n                       {'type': ['dict', 'string'],\n                        'validator': 'bulk_schema'}]} ")
    ...
    def __validate_dependencies_mapping(self, dependencies, field):
        ...
        for dependency_name, dependency_values in dependencies.items():
            if not isinstance(dependency_values, Sequence) or isinstance(dependency_values, _str_type):
    ...
    def _validate_excludes(self, excludes, field, value):
        ...
        for exclude in excludes:
            if exclude in self.schema and 'required' in self.schema[exclude] and self.schema[exclude]['required']:
    ...
    def _validate_items(self, items, field, values):
        ...
        if len(items) != len(values):
            ...
            schema = dict(((i, definition) for i, definition in enumerate(items)))
            validator = self._get_child_validator(document_crumb=field, schema_crumb=(field, 'items'), schema=schema)
            if not validator(dict(((i, value) for i, value in enumerate(values))), update=self.update, normalize=False):
    ...
    def _validate_nullable(self, nullable, field, value):
        ...
        if value is None:
            ...
            self._drop_remaining_rules('empty', 'forbidden', 'items', 'keyschema', 'min', 'max', 'minlength', 'maxlength', 'regex', 'schema', 'type', 'valueschema')
        return
    ...
    def _validate_keyschema(self, schema, field, value):
        ...
        if isinstance(value, Mapping):
            validator = self._get_child_validator(document_crumb=field, schema_crumb=(field, 'keyschema'), schema=dict(((k, schema) for k in value.keys())))
    ...
    def _validate_readonly(self, readonly, field, value):
        ...
        if readonly:
            ...
            has_error = errors.READONLY_FIELD in self.document_error_tree.fetch_errors_from(self.document_path + (field,))
    ...
    def __validate_required_fields(self, document):
        ...
        """
        try:
            required = set((field for field, definition in self.schema.items() if self._resolve_rules_set(definition).get('required') is True))
        ...
        missing = required - set((field for field in document if document.get(field) is not None or not self.ignore_none_values))
    ...
    def __validate_schema_mapping(self, field, schema, value):
        ...
        validator = self._get_child_validator(document_crumb=field, schema_crumb=(field, 'schema'), schema=schema, allow_unknown=allow_unknown)
        try:
    ...
    def __validate_schema_sequence(self, field, schema, value):
        ...
        validator = self._get_child_validator(document_crumb=field, schema_crumb=(field, 'schema'), schema=schema, allow_unknown=self.allow_unknown)
    ...
    def _validate_type(self, data_type, field, value):
        ...
        if not any((self.__get_rule_handler('validate_type', x)(value) for x in types)):
    ...
    def _validate_type_number(self, value):
        if isinstance(value, (_int_types, float)) and not isinstance(value, bool):
            return True
    ...
    def _validate_valueschema(self, schema, field, value):
        ...
        if isinstance(value, Mapping):
            validator = self._get_child_validator(document_crumb=field, schema_crumb=schema_crumb, schema=dict(((k, schema) for k in value)))
...
class InspectedValidator(type):
    ...
    def __init__(cls, *args):
        ...
        def attributes_with_prefix(prefix):
            return tuple((x.split('_', 2)[-1] for x in dir(cls) if x.startswith('_' + prefix)))
        ...
        for attribute in attributes_with_prefix('validate'):
            if attribute.startswith('type_'):
                ...
                cls.validation_rules[attribute] = cls.__get_rule_schema('_validate_' + attribute)
        ...
        for attribute in attributes_with_prefix('normalize'):
            if attribute.startswith('coerce_'):
                ...
                cls.normalization_rules[attribute] = cls.__get_rule_schema('_normalize_' + attribute)

        for rule in ('coerce', 'rename_handler'):
            x = cls.normalization_rules[rule]['oneof']
            x[1]['schema']['oneof'][1]['allowed'] = x[2]['allowed'] = cls.coercers

        cls.normalization_rules['default_setter']['oneof'][1]['allowed'] = cls.default_setters
        cls.rules = {}
    ...
    def __get_rule_schema(cls, method_name):
        ...
        if not result:
            warn("No validation schema is defined for the arguments of rule '%s'" % method_name.split('_', 2)[-1])
        return result
