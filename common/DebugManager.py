...
class __DebugManager(MultiUpdate):
    ...
    def createKey(self, keyDescription):
        if isinstance(keyDescription, KeyDescription):
            keySource = (keyDescription.name, keyDescription.group, keyDescription.type)
            dvKey = zlib.crc32(str(keySource))
...
def REMOVE_KEY(dvKey):
    if _GLOBAL_ACTIVITY:
        return __DebugManager().removeKey(dvKey)
    LOG_ERROR('Attempt to use Debug System not in development mode.')
