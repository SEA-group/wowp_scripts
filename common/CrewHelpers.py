...
CREW_NAME_TO_CREW_SPECIALIZATION = {'PILOT': SpecializationEnum.PILOT,
 'PILOT_01': SpecializationEnum.PILOT,
 'PILOT_02': SpecializationEnum.PILOT,
 'GUNNER1': SpecializationEnum.GUNNER,
 'GUNNER_01': SpecializationEnum.GUNNER,
 'GUNNER_02': SpecializationEnum.GUNNER,
 'NAVIGATOR': SpecializationEnum.NAVIGATOR}
DEFAULT_CREW_SUBSPECIALIZATION = 0
DEFAULT_CREW_BODY_TYPE = CREW_BODY_TYPE.MALE
DEFAULT_CREW_UPGRADE = 1
DEFAULT_BOT_AVATAR_BODY_TYPE = [{0: [DEFAULT_CREW_BODY_TYPE, 1, DEFAULT_CREW_UPGRADE]}]
...
def getPilotExtUpgrades(crewBodyType):
    if crewBodyType is None:
        return DEFAULT_CREW_UPGRADE
    else:
        for crewBody in crewBodyType:
            for spec, (bodyType, iconIndex, upgradeType) in crewBody.iteritems():
                if spec == SpecializationEnum.PILOT:
                    return upgradeType
...
def getCrewSpecialization(partsSettings, partTypes):
    spec = getCrewSpecializationByComponentName(partsSettings, partTypes)
    if not spec:
        spec = set()
        for partID, partDB in partsSettings.getPartsList():
            specialization = getCrewSpecializationByName(partDB.name)
            if specialization is not None:
                spec.add(specialization)

    return spec


def getCrewSpecializationByComponentName(partsSettings, partTypes):
