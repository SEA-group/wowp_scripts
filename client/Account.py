...
class PlayerAccount(BigWorld.Entity):
    ...
    def respondCommand(self, requestID, resultID, data):
        if hasattr(self.accountCmd, 'router'):
            self.accountCmd.router.onCmdResponse(requestID, resultID, data)
    ...
    def receivePlayerInfo(self, initPlayerInfoMap):
        ...
        if BWPersonality.g_lobbyInterview[0] and info.useGUIType != GUI_TYPES.EMPTY and BWPersonality.g_lastMapID != -1:
    ...
    def __handleActionListChanged(self, operation):
        ...
        if economics and BWPersonality.g_lobbyCarouselHelper.inventory.inventoryDataInitialized:
            for planeID in BWPersonality.g_lobbyCarouselHelper.inventory.getBoughtAircraftsList():
                ...
                BWPersonality.g_lobbyCarouselHelper.refreshAircraftData(planeID, True, force=True)
    ...
    def __checkUpdateTrainingRoom(self):
        while self.__updateTrainingRoomData.has_key(self.__updateTrainingSequence):
            ...
            if self._lobbyInstance != None:
                trainingRoomData = wgPickle.loads(wgPickle.FromServerToClient, trainingRoomDataPickled)
                ...
                if dataType == PREBATLE_DATA.ROOM_INFO:
                    ...
                    LOG_ERROR('updateTrainingRoom: unknown dataType', dataType, trainingRoomData)
    ...
    def onPrebattleArenaCreationFailed(self):
        self.stopWaitingForArena()
        if self._lobbyInstance != None:
            self._lobbyInstance.trainingRoomHelper.onPrebattleArenaCreationFailed()
        return
    ...
    def onBecomePlayer(self):
        ...
        """
        LOG_INFO('Account.onBecomePlayer()')
        ...
        if db.DBLogic.g_instance == None:
            ...
            if consts.IS_DEBUG_IMPORTED:
                if consts.QA_ACCOUNT_ENABLED or consts.QA_TESTING_ENABLED:
            ...
            CompoundSystem.reportBecomePlayer(True)
            return
    ...
    def _getWarActionHangar(self, currSpaceID):
        ...
        fraction = BWPersonality.gameParams.get('warAction', {}).get('fraction', consts.FRACTION.UNDEFINED)
        oldFraction = BWPersonality.gameParams.get('warAction', {}).get('oldFraction', consts.FRACTION.UNDEFINED)
        if fraction == consts.FRACTION.UNDEFINED:
            fraction = oldFraction
        hangar = None
        for ui in uiConfig.hangar:
            if warState in ui.state and fraction in ui.fraction:
                hangar = ui.name
                break
        ...
        if hangar is None:
            ...
            if curHangarSpace and 'spaceID' in curHangarSpace:
                return (curHangarSpace['hangarType'], curHangarSpace['spaceID'])
            LOG_WARNING('_getWarActionHangar: spaceID not found current Hangar:', hangar)
            return
            return
    ...
    def onBecomeNonPlayer(self):
        ...
        CompoundSystem.reportBecomePlayer(False)
        return
    ...
    def __isVSEReady(self):
        return g_hangarSpace.isVSEPlansStarted
        ...
        if self.__isVSEReady():
            if self.__queueVSEData:
                ...
                for event, param in self.__queueVSEData:
                    ...
                    BigWorld.callback(0, partial(PlayerAccount.sendVisualScriptEvent, event, param))
