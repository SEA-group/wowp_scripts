...
import db
from Helpers.iconPathHelper import ICON_MODULE_DEFAULT_PATH, get48ModuleIconPath
from adapters.DefaultAdapter import DefaultAdapter
from consts import UPGRADE_TYPE, MIN_CALIBER, EFFECTIVE_AGAINST_ARMORED_OBJECTS, UPGRADE_TYPE_TO_COMPONENT_TYPE, MODULE_GROUP_NAME
    ...
    def __call__(self, account, ob, **kw):
        if ob is None:
            ...
            if upgrade is not None:
                ...
                if gunData is None:
                    moduleType = localizeComponents(MODULE_GROUP_NAME.get(getattr(upgrade, 'moduleGroup', None), MODULE_GROUP_NAME[upgrade.type]))
                else:
                ...
                adaptedOb['icoPath'] = get48ModuleIconPath(getattr(upgrade, 'iconPath', ICON_MODULE_DEFAULT_PATH))
