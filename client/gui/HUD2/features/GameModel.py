...
from gui.HUD2.features.resourcePoints.ResourcePointsModel import ResourcePointsModel
...
from gui.HUD2.features.CombatLog.CombatLogModel import CombatLogModel

class GameModel(DataModel):
    SCHEME = Structure(time=TimeModel, arena=ArenaModel, loading=LoadingModel, crosshair=CrosshairModel, respawn=RespawnModel, deathInfo=DeathInfoModel, control=ControlModel, combatLog=CombatLogModel, parameters=ParametersModel, player=PlayerModel, domination=GameModeModel, systemInfo=SystemInfoModel, hotkeys=HotKeysModel, planeScheme=PlaneSchemeModel, ammunitions=AmmunitionModel, consumables=ConsumableModel, equipments=EquipmentModel, skills=SkillsModel, crews=CrewModel, battlePoints=BattleLogModel, battleNotifications=BattleNotificationsModel, battleAlerts=BattleAlertsModel, entities=EntitiesModel, target=TargetModel, ammoBelts=AmmoBeltsModel, sectorItems=SectorItemsModel, Intro=IntroModel, outro=OutroModel, spectator=SpectatorModel, fastCommand=FastCommandModel, measurementSystem=MeasurementModel, radar=RadarModel, hudSettings=HudSettingsModel, planeCharacteristics=PlaneCharacteristicsModel, denunciation=DenunciationModel, friends=FriendsModel, planes=BasePlaneDataModel, healthRepair=HealthRepairModel, currentPlayerInfo=PlayerInfoModel, ranks=RanksModel, achievements=AchievementsModel, classTasks=PlaneTypeObjectivesModel, battleReplay=BattleReplayModel, utils=UtilsModel, minimap=MinimapModel, attitudeIndicator=AttitudeIndicatorModel, gameplayHints=GameplayHintsModel, resourcePoints=ResourcePointsModel)