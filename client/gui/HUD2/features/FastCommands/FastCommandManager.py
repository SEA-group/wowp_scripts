...
from consts import MESSAGE_TYPE, GAME_OPTIONS_STATS
...
class FastCommandManager(RegicToHUDExecutionManager):
    ...
    def __init__(self, features, model):
        super(FastCommandManager, self).__init__()
        self._model = model
        self.__lastSendTime = 0
        self._targetsRequestMap = dict()
        self.__callID = 0
        ...
        self._chatCommandsMessage = {InputMapping.CMD_F2_CHAT_COMMAND: CommandTypes.AFFIRMATIVE,
         ...
         InputMapping.CMD_F8_CHAT_COMMAND: CommandTypes.INTENTIONS,
    ...
    def __subscribe(self):
        ...
        self._player.eLeaveWorldEvent += self._unsubscribe
    ...
    def onSelectTargetFromFlash(self, targetList, callID):
    ...
    def dispose(self):
        super(FastCommandManager, self).dispose()
        for command in InputMapping.CHAT_COMMANDS:
            self._processor.removeListeners(command, partial(self.__showChatCommand, command))
