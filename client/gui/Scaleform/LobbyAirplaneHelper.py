...
import db.DBLogic
import db.DBParts
from consts import BLOCK_TYPE, UPGRADE_TYPE, AIRCRAFT_CHARACTERISTIC, TURRET_DPS_TO_FIREPOWER_CFC, BULLET_FLY_DIST_CORRECTION
...
from Helpers.PerformanceSpecsHelper import getPerformanceSpecsTableDeprecated, PlaneCharacteristicsField, getCharacteristicsList, getGroupedDescriptionFields
...
class ModuleSpecsVO:
    ...
    TYPE_UP_SECTOR = 12
    TYPE_DOWN_SECTOR = 13
    LOWER_IS_BETTER = [TYPE_DOWN_SECTOR, TYPE_MASS]

    def __init__(self, specType, name, value = 0, unit = '', planeID = -1):
        ...
        self.higherIsBetter = self.specType not in ModuleSpecsVO.LOWER_IS_BETTER
        return

    def setCmpValue(self, cmpValue):
        if self.specType == ModuleSpecsVO.TYPE_CALIBER:
            self.comparisonValue = round(cmpValue, 2)
        else:
...
def getShellDescription(shell, measurementSystem = None):
    ...
    specs.append(ModuleSpecsVO(ModuleSpecsVO.TYPE_DAMAGE, localizeLobby('MODULES_CHARACTERISTICS_DAMAGE'), shell.explosionDamage))
    specs.append(ModuleSpecsVO(ModuleSpecsVO.TYPE_EXPLOSION_RADIUS, localizeLobby('MODULES_CHARACTERISTICS_EXPLOSION_RADIUS'), _mkint(ms.getMeters(shell.explosionRadius / consts.WORLD_SCALING)), ', {0}'.format(ms.localizeHUD('ui_meter'))))
    return specs
...
class LobbyAirplane(object):
    ...
    def __addAirplaneDescriptionToList(self, aircraftForCompare, installedGlobalID, modify, measurementSystem = None):
        ...
        return getCharacteristicsList(specsTable, installedGlobalID, compareSpecsTable, measurementSystem) + self.getWeaponDescriptionList()
