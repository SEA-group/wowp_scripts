...
from consts import GAME_MODE, INACTIVE_ACCOUNT_DISCONNECT_TIME, BLOCK_TYPE, EMPTY_IDTYPELIST, CLIENT_STATS_TYPE, WAITING_INFO_TYPE, CLIENT_UPDATE_STATS_PERIOD, CLIENT_UPDATE_HANGAR_PERIOD
...
import messenger
from Helpers.i18n import localizeOptions, getFormattedTime, localizeMessages, localizeLobby, localizeAchievements
from gui.Scaleform.WebBrowser2 import WebBrowser, setWebBrowserSize
...
from clientConsts import BATTLE_NAME_BY_TYPE_HUD_LOC_ID, BATTLE_LOBBY_DESC_BY_TYPE_HUD_LOC_ID, BATTLE_LOBBY_ICON_BY_TYPE_HUD_LOC_ID, KEY_RESEARCH_TREE_NATION, KEY_RESEARCH_TREE_NATION_LIST, KEY_RESEARCH_TREE_DEV_NATION_LIST, HANGAR_MODE, CLASTERS, SWITCH_STYLES_BUTTONS, HANGAR_LOBBY_WAITING_SCREEN_MESSAGE
import InputMapping
...
class Lobby(GUIWindowAccount):
    ...
    def __init__(self):
        ...
        Lobby.BATTLE_TYPE_LIST = [consts.ARENA_TYPE.NORMAL, consts.ARENA_TYPE.TRAINING]
    ...
    def initialized(self):
        ...
        self.addExternalCallbacks({'hangar.GetAirplanesList': self.__lobbyCarouselHelper.getAirplanesListRequest,
         ...
         'hangar.ShowAnimation': self.onShowAnimation,
         ...
         'hangarMenu.Archive': self.onArchive,
         'hangarMenu.BuyGold': self.onBuyGold,
         'hangarMenu.PremiumShop': self.onPremiumShop,
         ...
         'shop.gotoDunkerk': self.onGotoDunkerkShop,
         'browser.gotoLTO_IL_1': self.onGotoLTO_IL_1,
         'browser.gotoSecretSanta': self.onGotoSecretSanta})
    ...
    def __onBrowser2Initialize(self, width, height, linkageName, callbacks):
        if self.__browser2 is None:
            setWebBrowserSize(width, height)
            self.__browser2 = WebBrowser(self.movie, linkageName, callbacks.__dict__, self)
        return

    def __onBrowser2Dispose(self):
        if self.__browser2 is not None:
    ...
    def __onBuyPlaneResponseCB(self, planeID):
        GameSound().ui.postEvent('Play_ui_buy_plane')
        self.__lobbyCarouselHelper.inventory.openAircraft(planeID)

    def onBuyPlaneResponse(self, planeID):
        if self.__lobbyCarouselHelper.inventory.inventoryDataInitialized:
            self.__onBuyPlaneResponseCB(planeID)
        else:
            self.__lobbyCarouselHelper.inventory.syncAircraftsData([planeID], partial(self.__onBuyPlaneResponseCB, planeID))
    ...
    def onPremiumShop(self):
        import BWPersonality
        url = Settings.g_instance.scriptConfig.urls[WebPageHolder.URL_PREMIUM_SHOP]
        WebPageHolder().openUrl(url)

    def onArchive(self, spaID = None):
        LOG_DEBUG('onArchive', spaID)
        import BWPersonality
        spaID = spaID or BWPersonality.g_initPlayerInfo.databaseID
        url = Settings.g_instance.scriptConfig.urls[WebPageHolder.URL_ARCHIVE]
        url = url % {'spa_id': str(spaID)}
        WebPageHolder().openUrl(url)
    ...
    def onGotoDunkerkShop(self):
        url = Settings.g_instance.scriptConfig.urls[WebPageHolder.URL_DUNKERK_SHOP]
        WebPageHolder().openUrl(url)

    def onGotoLTO_IL_1(self):
        url = Settings.g_instance.scriptConfig.urls[WebPageHolder.URL_LTO_IL_1]
        WebPageHolder().openUrl(url)

    def onGotoSecretSanta(self):
        url = Settings.g_instance.scriptConfig.urls[WebPageHolder.URL_SECRET_SANTA]
        WebPageHolder().openUrl(url)
    ...
    def onShowAnimation(self, animationType, animationParam):
        animationMap = {'bomber': lambda x: BigWorld.player().callVSE('onBomberBuilt', dict(plane_id=int(x)))}
        animationCMD = animationMap.get(animationType, None)
        if animationCMD is not None:
            animationCMD(animationParam)
        else:
            LOG_ERROR('Lobby:onShowAnimation wrong animationType', animationType, animationParam)
        return
    ...
    def __fillBattleTypeList(self):
        ...
        battleTitleSingleVO.container = [consts.ARENA_TYPE.NORMAL]
    ...
    def __getKeyMapping(self, key):
        from gui.Scaleform.UIHelper import getKeyLocalization
        return getKeyLocalization(getattr(InputMapping, key, None))
