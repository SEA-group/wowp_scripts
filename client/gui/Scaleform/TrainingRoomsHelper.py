...
class RoomMapsVO:
    ...
    def __init__(self):
        self.mapID = 1
        self.schemeName = 'Item'
        self.mapName = 'GroupName'
...
class TrainingRoomHelper:
    ...
    def __init__(self, account):
        ...
        self.__lastRoomSearchFilters = {'mapIDs': consts.PREBATTLE_MAPID_UNDEFINED_FILTER,
    ...
    def onRefreshRoomList(self, mapIDs, gameModeID, fireModeID, levelTypeID):
        LOG_DEBUG('onTrainingRoomListRequest', mapIDs, gameModeID, fireModeID, levelTypeID)
        self.__lastRoomSearchFilters = {'mapIDs': mapIDs,
         'gameModeID': gameModeID,
         'fireModeID': fireModeID,
         'levelTypeID': levelTypeID}
        self.__getAccountCmd().refreshRoomList(mapIDs, gameModeID, fireModeID, levelTypeID)
        self.positionInUI = self.TrainingRoomUI.IN_LIST_ROOM
    ...
    def refreshRoomListForChangePlane(self, planeID, planeLevel):
        ...
        if self.__roomsListActive and not self.__roomCreateActive and self.positionInUI == TrainingRoomHelper.TrainingRoomUI.IN_LIST_ROOM:
            updateRooms = [ self.__convertRoomToFlashVO(room) for room in self.__roomList ]
            try:
    ...
    def refreshRoomListResponse(self, roomList):
        ...
        if self.__roomsListActive and not self.__roomCreateActive:
            ...
            if toAddRoomIDs:
                addRooms = [ self.__convertRoomToFlashVO(newRoomDict[roomID]) for roomID in toAddRoomIDs ]
                self.__account.call_1('trainingRoomsList.AddRooms', addRooms)
            if toUpdateRoomIDs:
                updateRooms = [ self.__convertRoomToFlashVO(newRoomDict[roomID]) for roomID in toUpdateRoomIDs ]
                self.__account.call_1('trainingRoomsList.UpdateRooms', updateRooms)
        self.__roomList = roomList
        self.__account.call_1('trainingRoomsList.roomListResponseEnd')
    ...
    def __convertRoomToFlashVO(self, room):
        ...
        flashVO.gameMode = localizeHUD(consts.PREBATTLE_GAMEMODES_FILTER_MAP.get(extract(room, 'gameMode')))
        ...
        planeLevelFrom = localizeHUD(consts.PREBATTLE_PLANES_LEVELS.PREBATTLE_PLANES_LEVELS_MAP.get(flashVO.planeLevelFrom))
        planeLevelTo = localizeHUD(consts.PREBATTLE_PLANES_LEVELS.PREBATTLE_PLANES_LEVELS_MAP.get(flashVO.planeLevelTo))
        flashVO.planeLevel = planeLevelFrom + ' - ' + planeLevelTo
        flashVO.fireMode = localizeLobby(consts.PREBATTLE_FIRE_MODE_FILTER.PREBATTLE_FIRE_MODE_FILTER_MAP.get(extract(room, 'fireMode')))
    ...
    def updatePrebattleCreateRoomAvailableFilters(self):
        if self.__roomCreateActive == True:
            ...
            self.onRefreshRoomList(self.__lastRoomSearchFilters['mapIDs'], self.__lastRoomSearchFilters['gameModeID'], self.__lastRoomSearchFilters['fireModeID'], self.__lastRoomSearchFilters['levelTypeID'])
    ...
    def generateMapASList(self, filtersMap):
        ...
        for mapID in filtersMap['mapFilters']:
            ...
            if arenaData:
                ...
                roomMapVO.mapName = localizeMap(arenaData.trainingRoomName)
    ...
    def __setLocalizedRoomSettings(self, roomMap):
        if self.__roomTeamsActive:
            flashVO = self.__convertRoomToFlashVO(roomMap)
            self.__account.call_1('trainingsRoomTeams.updateRoomData', flashVO)
    ...
    def __setLocalizedRoomList(self, roomList):
        if self.__roomsListActive:
            roomASList = [ self.__convertRoomToFlashVO(roomMap) for roomMap in roomList ]
    ...
    def onPrebattleArenaCreationFailed(self):
        self.__ShowErrorMessageBox(TRAINING_ROOM_REASON.ERROR_MESSAGE[TRAINING_ROOM_REASON.FAILED_TO_CREATE_ARENA])
    ...
    def __ShowErrorMessageBox(self, error_msg):
        ...
        self.__account.call_1('hangar.showErrorMessage', _error_local, localizeLobby('BATTLE_TRAINING_FLIGHT'))
