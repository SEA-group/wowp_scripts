...
SKIP_OUTPUT_KEYS = ['requestOwnerInfo',
 'SpaceMove',
 'receive',
 'browser.mouse']
...
def _process_list(somelist):
    for i, v in enumerate(somelist):
        if isinstance(v, dict):
            somelist[i] = _process_dict(v)
        elif isinstance(v, list):
            somelist[i] = _process_list(v)

    return somelist


def _process_dict(somedict):
    classname = somedict.pop('classtag', None)
    for k, v in somedict.iteritems():
        if isinstance(v, dict):
            somedict.pop(k, None)
            somedict[k] = _process_dict(v)
        elif isinstance(v, list):
            somedict.pop(k, None)
            somedict[k] = _process_list(v)
...
def ConvertJsonToPyobject(args_json):
    ...
    for arg in temp:
        if isinstance(arg, dict):
            args.append(_process_dict(arg))
        elif isinstance(arg, list):
            args.append(_process_list(arg))
        else:
            args.append(arg)
...
def ConvertPyobjectToJson(object):
    return json.dumps(object, default=_default_handler)
