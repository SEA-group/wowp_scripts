...
class LobbyCarouselHelper:
    ...
    def refreshAircraftData(self, aircraftID, sendASCallbacks, force = False):
        """
        ...
        if carouselAirplane is not None:
            self.__updateCarouselAirplane(carouselAirplane, self.inventory.getCarouselAircraft(aircraftID), force=force)
    ...
    def onSelectedPlaneID(self, planeID, reload3DModel = True):
        ...
        if planeID == -1:
            ...
            self.__planeSelected = None
            self.__onPlaneSelectedVSE(None)
            if self.__lobby is not None:
        ...
        if self.__planeSelected is not None:
            self.__onPlaneSelectedVSE(planeID)
    ...
    def __onPlaneSelectedVSE(self, planeID):
        from Account import PLANE_BLOCK_TYPE
        nationID = db.DBLogic.g_instance.getNationIDbyAircraftID(planeID) if planeID is not None else -1
        blockType = PLANE_BLOCK_TYPE.get(planeID, -1) if planeID is not None else -1
        params = dict(plane_id=planeID, nation_id=nationID, block_type=blockType)
        BigWorld.player().callVSE('onPlaneSelected', params)
        return
