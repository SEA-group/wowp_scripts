...
def prepareControlsGroup(mapping, groupID, controls):
    for cmdID, record in mapping.iteritems():
        ...
        if cmdLabel is not None and not isHide and not ((cmdID == InputMapping.CMD_PUSH_TO_TALK or cmdID == InputMapping.CMD_PUSH_TO_TALK_SQUAD or cmdID == InputMapping.CMD_TOGGLE_ARENA_VOICE_CHANNEL) and not VOIP.api().voipSupported):
