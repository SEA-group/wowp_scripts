...
class CameraStateFree(CameraState):
    ...
    def clearCameraTargetShift(self):
        self.__targetShifted = False
        self.strategy.clearCameraTargetShift()

    def exit(self):
        self.clearCameraTargetShift()
...
class HangarCamera(object):
    ...
    def setState(self, cameraType, params = None, replayMode = False):
...
class HangarShootingAvatar(object):
    ...
    def __init__(self, hangarSpace, planeID):
        ...
        self.onStateChanged = Event(self.em)
    ...
    def autoAim(self, historyLayer, bulletStartPos, reductionDir, bulletSpeed, autoguiderMaxDist, bulletTime, groundAngle, groupAngle, angleMod):
    ...
    def addBullet(self, startPos, endPos, bulletSpeed, time, gun, explosionEffect, delay = 0, gunID = 0, dPos = None):
        ...
        self.__hangarSpace.getModelManipulator().onAddBullet(gunID, delay)
        explosionF = None
        bullet = BigWorld.addBullet(startPos, endPos, bulletSpeed, time, gun.shootInfo.bulletRenderType, BULLET_PARAM.OWN, explosionF)
        self.__hangarSpace.getModelManipulator().onAddBullet(gunID, delay, 'SHELL_EFFECT????')
        return bullet
...
class ClientHangarSpace():
    ...
    def __init__(self):
        ...
        self.__isKeyEventSubsribed = True
        return
    ...
    def setCustomization(self, camouflages):
        if self.__modelManipulator:
            from _camouflages_data import generateID, camouflageTypeEnum
            camouflage = camouflages[CAMOUFLAGE_GROUPS.HULL]
            camouflageID = generateID(self.__vehicleInfo.planeID, camouflage, camouflageTypeEnum.HULL)
            import BWPersonality
    ...
    def recreateVehicle(self, vehicleInfo, loadedCallback, dbgHangarFlyingMode = False):
        LOG_TRACE('ClientHangarSpace::recreateVehicle', vehicleInfo)
        ...
        oldControllers = (self.__modelManipulator,
         self.__shellController,
         self.__weaponController,
         self.__turretController)
        callback = partial(BigWorld.callback, 0.05, partial(self.__onVehicleLoaded, loadedCallback, vehicleInfo, oldControllers))
        ...
        if self.__dummyDebugAvatar is None:
            self.__dummyDebugAvatar = HangarShootingAvatar(self, self.__planeID)
        controllersData = DestructibleObjectFactory.createControllers(BigWorld.player().id, self.__settings, self.__settings.airplane, partTypes, partStates, weaponsSlotsStates, self.__dummyDebugAvatar, fullLoading, callback, turretsName=[self.__turretName])
    ...
    def __onVehicleLoaded(self, callBack, vehicleInfo, oldControllers):
        LOG_TRACE('ClientHangarSpace::__onVehicleLoaded')
        _destroyControllers(oldControllers)
        if self.__modelManipulator:
            self.removeVehicle()
            ...
            BigWorld.addShadowEntity(self.__modelManipulator.compoundID, False)
    ...
    def destroy(self):
        ...
        if self.__waitCallback is not None and not self.spaceLoaded():
            ...
            self.removeVehicle()
            _destroyControllers((self.__modelManipulator,
             self.__shellController,
             self.__weaponController,
             self.__turretController))
            self.__modelManipulator = None
    ...
    def handleKeyEvent(self, event):
        if self.spaceLoaded():
            VSHangarEvent.onHangarKeyEvent(event)
        return False
...
def _destroyControllers(controllers):
