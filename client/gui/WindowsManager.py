...
class WindowsManager(object):
    ...
    def start(self):
        ...
        GlobalEvents.onMouseEvent += self.handleMouseEvent
        GlobalEvents.onAxisEvent += self.handleAxisEvent
        if not BigWorld.isRunningInEditor():
    ...
    def tryStartReplay(self):
        LOG_INFO('tryStartReplay')
        InputMapping.g_instance.onProfileLoaded -= self.tryStartReplay
        import BattleReplay
        if not BattleReplay.g_replay.startAutoPlay() and not consts.IS_OFFLINE_MODE_ENABLED:
            if not WgcLogin.try_to_login_with_wgc_api():
                self.showLogin()
    ...
    def showBattleUI(self):
        ...
        self.__battleUI = weakref.proxy(self.__screenManager.activeMovie)
    ...
    def showOfflineHUD(self):
        self.__screenManager.loadMovie(main_interfaces.GUI_SCREEN_OFFLINE_HUD)
