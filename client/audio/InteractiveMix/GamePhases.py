...
class GamePhases:
    ...
    def __onAvatarEnterWorld(self, avatar):
        avatar.eOnEntityStateChanged += self.__onAvatarStateChanged
        self.__setGamePhase(self.__getFightPhase())
    ...
    def __onAvatarLeaveWorld(self, avatar):
        self.__setGamePhase(self.__getFightPhase(FLAG.LEAVE_WORLD))
    ...
    def __getFightPhase(self, flag = FLAG.NONE):
        if self.__currentSoundPhase in (SOUND_PHASES.RESULT_SCREEN, SOUND_PHASES.SCREEN) or self.__currentSoundPhase in (SOUND_PHASES.SPECTATOR, SOUND_PHASES.NOSEDIVING) and not flag == FLAG.RESPAWN:
            return
    ...
    def __setGamePhase(self, localID):
        ...
        audio.debug.LOG_INFO('%s %s %s' % (audio.debug.DEBUG_AUDIO_TAG, 'SoundPhase: ', localID))
    ...
    @property
    def curretnSoundPhase(self):
        return self.__currentSoundPhase