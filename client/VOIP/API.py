...
class API(object):
    ...
    def setAvatarMuted(self, avatarID, muted):
        ...
        """
        try:
            avatarId = avatarID if isinstance(avatarID, (int, long)) else long(avatarID)
        except ValueError:
            avatarId = None
    ...
    def __onMemberStateChanged(self, dbid, iconID):
        self.__refreshVolume()
        isVisible = self.__channelsMgr.muteList is None or dbid in self.__channelsMgr.muteList
        for type, observer in self.__memberStateObservers:
            self.__channelsMgr.outbox.visualizeMemberState(dbid, iconID, isVisible, type, observer)
