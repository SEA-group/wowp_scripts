...
import db.DBLogic
import ResMgr
from consts import INPUT_SYSTEM_STATE, INPUT_SYSTEM_PROFILES_LIST, DB_PATH, MEASUREMENT_SYSTEMS, RADAR_STATES, RADAR_SIZES, RADAR_LOCK_ROTATION, ATTITUDES_STATES
...
GAME_UI_MAIN_KEYS = {'measurementSystem': 0,
 ...
 'deflectionTest1': 3,
 'radarState': 2,
 'radarStates': ','.join(RADAR_STATES),
 'radarSize': 2,
 'radarSizes': ','.join(RADAR_SIZES),
 'radarLockRotation': 1,
 'radarLocks': ','.join(RADAR_LOCK_ROTATION),
 'showDefendersActive': False,
 'hudMinimapSize': 1,
 'attitudeMode': 0,
 'attitudeModes': ','.join(ATTITUDES_STATES)}
...
SOUND_PARAMETERS = {'isMasterVolume': 'master',
 ...
 'isGunsVolume': 'guns'}
SOUND_VOLUME_PARAMETERS = {'master': 0.5,
 'music': 0.5,
 'ui': 0.5,
 'aircraft': 0.5,
 'sfx': 0.5,
...
VOIP_PARAMETERS_DICT = {'enableArenaVoiceChat': 'autoConnectArenaChannel',
 'voiceChatVoiceVolume': 'masterVolume',
 'voiceChatMicrophoneSensitivity': 'micVolume',
 'voiceChatAmbientVolume': 'fadeVolume',
 'enableVoiceChat': 'isVoipEnabled',
 'voiceChatMicDevice': 'captureDevice'}
...
class Settings(object):
    ...
    def __init__(self, scriptConfig, engineConfig, userPrefs, customGraphicPrefs):
        ...
        self.eFastFM = Event()
        self.eShowDefendersChanged = Event()
        self.eRadarLockChanged = Event()
        self.eRadarSizeChanged = Event()
        self.eRadarStateChanged = Event()
        self.eAttitudeModeChanged = Event()
        self.__gameUIEvents = {'alternativeColorMode': self.eAlternativeColorModeEnabled,
         'collisionWarningSystem': self.eCollisionWarningSystemEnabled,
         'navigationWindowRadar': self.onNavWindowListChanged,
         'navigationWindowMinimap': self.onNavWindowListChanged,
         'combatInterfaceType': self.eCombatInterfaceType,
         'mainDevices': self.eMainDevicesVisibility,
         'horizonList': self.eAviaHorizonType,
         'curPlayerListState': self.ePlayerListType,
         'isSniperMode': self.eSetSniperMode,
         'measurementSystem': self.onMeasurementSystemChanged,
         'battleChatUnVisible': self.eBattleChatSetVisible,
         'isChatEnabled': self.eGameChatEnabled,
         'fastFM': self.eFastFM,
         'showDefendersActive': self.eShowDefendersChanged,
         'radarLockRotation': self.eRadarLockChanged,
         'radarState': self.eRadarStateChanged,
         'radarSize': self.eRadarSizeChanged,
         'attitudeMode': self.eAttitudeModeChanged}
        self.__mainEvents = {'cameraEffectsEnabled': self.eCameraEffectsSetEnabled,
         'maxMouseCombatFov': self.eMaxMouseCombatFovChanged}
        ...
        if data.name in MarkerSettings.MARKER_TARGET_TYPE:
            ...
            for prop in param.values():
                mDict[param.name][prop.name] = {'label': Settings.loadData(prop, 'label', '__empty__'),
                 'tooltip': Settings.loadData(prop, 'tooltip', '__empty__'),
                 'list': prop.readStrings('num')}
    ...
    def __loadHangarSpaceSettings(self):
        ...
        for item in next((pref for name, pref in self.userPrefs.items() if name == KEY_HANGAR_SETTINGS), {}).items():
            ...
            itemDict.update({k:(v.asString if k != 'ignoreEventHangar' else v.asBool) for k, v in item[1].items()})
    ...
    def setGraphicsDetailsSD(self):
        ...
        if graphicsDetail is not None:
            sdGraphicsSettings = {graphicsDetailName:graphicsDetailValue for graphicsDetailName, graphicsDetailValue in graphicsDetail.iteritems()}
    ...
    def setGraphicsDetails(self, graphicsQualityName):
        ...
        BigWorld.savePreferences()
        BigWorld.profilerEvent('Graphics preset commit: ' + graphicsQualityName)
    ...
    def getVideoResolutions(self):
        ...
        for i, mode in enumerate(filter(self._isVideoModeCorrect, modes)):
            modes_list.append(mode[4])
            if mode[4] == actualMode:
                currentResolution = i

        if currentResolution == -1:
            modes_list.append(actualMode)
            currentResolution = len(modes_list) - 1
        return (modes_list, currentResolution)
        ...
        if res:
            for profilePresetObject in res.values():
                ...
                for presetObject in profilePresetObject.values():
                    d[profilePresetObject.name].append({'name': presetObject.name,
                     'localizationID': presetObject.asString})

            ResMgr.purge(INPUT_PROFILES_PRESETS_PATH, True)
        return d

    def _loadCurrentInputProfilesPresets(self):
        return {profileName:Settings.loadData(self.userPrefs, KEY_INPUT_PROFILES_PRESETS + profileName, profileObject[0]['name']) for profileName, profileObject in self.inputProfilesPresets.iteritems()}
    ...
    def loadGameUI(self):
        return {key:Settings.loadData(self.userPrefs, KEY_GAME_UI + key, defaultValue) for key, defaultValue in GAME_UI_MAIN_KEYS.items()}
    ...
    def getAimsDataByProfile(self, profileName):
        if profileName in self.aims:
            return {key:self.aims[profileName][key] for key in AIMS_KEYS.keys()}
        else:
    ...
    def _loadVoipSettings(self):
        return {key:Settings.loadData(self.userPrefs, KEY_VOIP_SETTINGS + key, defaultValue) for key, defaultValue in VOIP_KEYS.items()}
    ...
    def loadData(root, sectionName, defaultValue):
        ...
        elif isinstance(defaultValue, unicode):
            return root.readString(sectionName, defaultValue)
        elif isinstance(defaultValue, int):
            return root.readInt(sectionName, defaultValue)
        else:
            LOG_ERROR('load - undefined type of defaultValue', sectionName, defaultValue)
            return
            return
    ...
    def loadSoundPrefs(self):
        self._soundPrefs['volume'] = {categoryName:self.userPrefs.readFloat(KEY_SOUND_PREFERENCES + categoryName, defaultValue) for categoryName, defaultValue in SOUND_VOLUME_PARAMETERS.iteritems()}
        self._soundPrefs['volumeEnabled'] = {volumeKey:self.userPrefs.readBool(KEY_SOUND_PREFERENCES_ENABLED + settKey, True) for settKey, volumeKey in SOUND_PARAMETERS.iteritems()}
    ...
    def _loadXmppChatSettings(self):
        return {key:Settings.loadData(self.userPrefs, KEY_XMPP_CHAT_SETTINGS + key, defaultValue) for key, defaultValue in XMPP_CHAT_KEYS.iteritems()}
    ...
    def getScreenShotDirectory(self):
        return BigWorld.getScreenShotDirectory()
    ...
    def _loadReplaySettings(self):
        return {key:Settings.loadData(self.userPrefs, KEY_REPLAY_PREFERENCES + key, defaultValue) for key, defaultValue in REPLAY_KEYS.iteritems()}
...
class ScriptConfig():
    ...
    def __init__(self, data):
        ...
        self.urls[WebPageHolder.URL_FORUM] = data.readString('url_forum', 'http://forum-ru.worldofwarplanes.com/')
        self.urls[WebPageHolder.URL_SEND_ERROR] = data.readString('url_send_error', 'https://ru.wargaming.net/support/kb/categories/2')
        self.urls[WebPageHolder.URL_FORGOT_PASSWORD] = data.readString('url_forgot_password', 'https://worldoftanks.ru/personal/password_reset/new/')
        self.urls[WebPageHolder.URL_ACHIEVEMENTS] = data.readString('urlAchievements', 'http://worldofwarplanes.ru/community/accounts/%s/')
        self.urls[WebPageHolder.URL_TOKENS_HELP] = data.readString('urlTokensHelp', 'http://worldofwarplanes.ru/')
        self.urls[WebPageHolder.URL_BUY_GOLD] = data.readString('urlBuyGold', 'https://ru.wargaming.net/shop/gold/?user=%(userEncoded)s')
        self.urls[WebPageHolder.URL_BUY_QUEST_CHIPS] = data.readString('urlBuyQuestChips', 'https://ru.wargaming.net/shop/wowp/combat_task/')
        self.urls[WebPageHolder.URL_DUNKERK_SHOP] = data.readString('url_Dunkerk_shop', 'http://worldofwarplanes.ru/news/dunkirk-july-2017-premium/')
        self.urls[WebPageHolder.URL_PREMIUM_SHOP] = data.readString('urlPremiumShop', 'https://ru.wargaming.net/shop/wowp/main/')
        self.urls[WebPageHolder.URL_ARCHIVE] = data.readString('urlProfileArchive', 'https://ru.wargaming.net/archive_page_link/')
        self.urls[WebPageHolder.URL_LTO_IL_1] = data.readString('url_lto_il_1', 'https://ru.wargaming.net/')
        self.urls[WebPageHolder.URL_SECRET_SANTA] = data.readString('urlSecretSanta', 'https://worldofwarplanes.ru/news/secret-ded-moroz-2017/?utm_source=wowp_game&utm_campaign=secret_santa')
        self.urls['urlKong'] = data.readString('urlKong', '')
        self.urls[WebPageHolder.URL_REGISTRATION] = data.readString('url_registration', '')
        ...
        self.timeFormated = {'dBAHMS': data.readString(KEY_TIME_FORMATED % 'dBAHMS', '%d %B, %A, %H:%M:%S'),
         'dmY': data.readString(KEY_TIME_FORMATED % 'dmY', '%d.%m.%Y'),
         'dBYAHMS': data.readString(KEY_TIME_FORMATED % 'dBYAHMS', '%d %B %Y %A, %H:%M:%S'),
         'dbYHMS': data.readString(KEY_TIME_FORMATED % 'dbYHMS', '%d %b %Y, %H:%M:%S'),
         'default': data.readString(KEY_TIME_FORMATED % 'default', '%d %B, %H:%M:%S'),
         'dmYHM': data.readString(KEY_TIME_FORMATED % 'dmYHM', '%d.%m.%Y %H:%M')}
        return
