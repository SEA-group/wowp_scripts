...
def onStateEdit(event):
    ...
    warAction['fraction'] = event.ob['fraction']
    warAction['oldFraction'] = event.ob['oldFraction']
...
def onFractionEdit(event):
    import BWPersonality
    player = BigWorld.player()
    warAction = BWPersonality.gameParams.setdefault('warAction', {})
    currFraction = warAction['fraction']
    warAction['fraction'] = event.ob['fraction']
    warAction['oldFraction'] = event.ob['oldFraction']
    if currFraction != event.ob['fraction']:
        player.updateHangarSpace()