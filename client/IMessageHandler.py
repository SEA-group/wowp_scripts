...
from Helpers.i18n import localizeMessages, getFormattedTime, localizeChat, localizeUpgrade, localizeAchievements
...
from Helpers.i18n import separateThousandths
...
def goldReceived(imsg):
    ...
    imsg['msgData'] = dict(msgBody=localizeMessages('LOBBY_MSG_DAILY_WIN'), gold=__localizeValuePositive(gold))
    return imsg
...
def premBought(imsg):
    ...
    if gold:
        imsg['msgData']['gold'] = __localizeValueNegative(gold)
    if tickets:
        imsg['msgData']['tickets'] = __localizeNegative(tickets)
    return imsg
...
def __resbattleMsg(battleResult):
    if battleResult['winState'] == consts.WIN_STATE.LOSE:
        action = HtmlMessagerTypes.MESSAGE_LOSE
        msgHeader = localizeBattleResults('AFTERBATTLE_STATISTICS_MESSAGE_LOST')
    elif battleResult['winState'] == consts.WIN_STATE.WIN:
        action = HtmlMessagerTypes.MESSAGE_WIN
        msgHeader = localizeBattleResults('AFTERBATTLE_STATISTICS_MESSAGE_WIN')
    else:
        action = HtmlMessagerTypes.MESSAGE_DRAW
        msgHeader = localizeBattleResults('AFTERBATTLE_STATISTICS_MESSAGE_DRAW')
    arenaStartTime = getFormattedTime(battleResult['startTime'], Settings.g_instance.scriptConfig.timeFormated['dBAHMS'])
    arenaData = db.DBLogic.g_instance.getArenaData(battleResult['mapID'])
    arenaName = '{}: {}'.format(localizeMap(arenaData.trainingRoomName), localizeMap(arenaData.trainingRoomSecondName))
    battleResult['arenaName'] = arenaName
    msgData = dict(msgBody=arenaName + ' (' + arenaStartTime + ')', action=action, arenaType=battleResult['arenaType'])
    msgData.update(((k, battleResult[k]) for k in ['totalXP',
     'totalFreeXP',
     'repairCr',
     'refill']))
    msgData['totalCr'] = battleResult['actualCr']
    msgData['quests'] = battleResult['quests']
    msgData['achievements'] = {}
    for team in battleResult['teams'].itervalues():
        for member in team.itervalues():
            if member['id'] != battleResult['playerID']:
                continue
            msgData['achievements'].update(member['achievements'])
            for plane in member['planes']:
                for id_, data in plane['achievements'].iteritems():
                    if id_ in msgData['achievements']:
                        msgData['achievements'][id_]['bonuses'].extend(data['bonuses'])
                    else:
                        msgData['achievements'][id_] = data

    return (msgHeader, msgData)


def resbattle(imsg):
    battleResult = imsg['msgData']
    reportID = battleResult['startTime']
    imsg['reportID'] = str(reportID)
    imsg['autoShow'] = battleResult['autoShow']
    arenaData = db.DBLogic.g_instance.getArenaData(battleResult['mapID'])
    battleResult['arenaName'] = localizeMap(arenaData.typeName)
    from BattleReplay import g_replay
    if g_replay is not None:
        g_replay.onBattleResultsReceived(battleResult)
    for team in battleResult['teams'].itervalues():
        for member in team.itervalues():
            member['name'] = getBotName(member['name'], _airplanesConfigurations_db.getAirplaneConfiguration(member['firstGID']).planeID)

    from exchangeapi.AdapterUtils import getAdapter
    getAdapter('IBattleResult', ['battleResult']).add(None, None, battleResult, reportID=reportID)
    generateEvent('add', 'add', 'IBattleResult', [[reportID, 'battleResult']], None, battleResult, None)
    imsg['msgHeader'], imsg['msgData'] = __resbattleMsg(battleResult)
    return imsg
...
def buy(imsg):
    ...
    if credits:
        imsg['msgData']['credits'] = __localizeValueNegative(credits)
    elif gold:
        imsg['msgData']['gold'] = __localizeValueNegative(gold)
    elif tickets:
        imsg['msgData']['tickets'] = __localizeValueNegative(tickets)
    return imsg
...
def sell(imsg):
    msgData = imsg['msgData']
    imsg['msgHeader'] = localizeMessages('LOBBY_HEADER_BOUGHT')
    imsg['msgData'] = dict(msgBody=localizeMessages('LOBBY_MSG_AIRCRAFT_SELL'), aircraftName=__getAircraftName(msgData['aircraftID']), isPremium=db.DBLogic.g_instance.isPlanePremium(msgData['aircraftID']), credits=__localizeValuePositive(msgData['price']), resultID=msgData['resultID'])
    return imsg
...
def research(imsg):
    ...
    imsg['msgHeader'] = localizeLobby('MESSAGE_EXPLORATION')
    imsg['msgData'] = dict(msgBody=localizeMessages('LOBBY_MSG_AIRCRAFT_INVESTIGATED'), aircraftName=__getAircraftName(msgData['aircraftID']), credits=__localizeNegative(allExp), expPlane=__localizeNegative(msgData['planeExp']), expFree=__localizeNegative(msgData['freeExp']))
    return imsg
...
def goldSpent(imsg):
    ...
    imsg['msgData'] = dict(msgBody=localizeLobby('LOBBY_BUBBLE_GOLD_EXCHANGE_SUCCESFULL_CH') if IS_CHINA else localizeMessages('LOBBY_BUBBLE_GOLD_EXCHANGE_SUCCESFULL'), gold=__localizeValueNegative(imsg['msgData']))
    return imsg
...
def ticketsReceived(imsg):
    ...
    imsg['msgData']['tickets'] = __localizePositive(tickets)
...
def creditsReceived(imsg):
    ...
    imsg['msgData'] = dict(msgBody='', action=HtmlMessagerTypes.MESSAGE_BUY, credits=__localizeValuePositive(imsg['msgData']))
    return imsg
...
def exchangeExp(imsg):
    imsg['msgHeader'] = localizeMessages('LOBBY_HEADER_BOUGHT')
    imsg['msgType'] = MESSAGE_TYPE.BUY_AIRCRAFT
    imsg['msgData'] = dict(msgBody=localizeMessages('LOBBY_BUBBLE_FREE_XP_CONVERTED').format(number=imsg['msgData']['exp']), gold=__localizeValueNegative(imsg['msgData']['gold']))
    return imsg
...
def installPreset(imsg):
    ...
    if resources['credits'] > 0:
        creditsMessage = __localizeValueNegative(resources['credits'])
    else:
        creditsMessage = ''
    if resources['exp'] > 0:
        expMessage = __localizeNegative(resources['exp'])
    else:
        expMessage = ''
...
def squadExcluding(imsg):
    body = localizeLobby('WING_MESSAGE_WING_DELETED_BY_TIMEOUT')
    imsg['msgHeader'] = localizeLobby('WING_HEADER_WING')
    imsg['msgData'] = dict(msgBody=body)
    return imsg
...
def researchUpgrade(imsg):
    ...
    imsg['msgData'] = dict(msgBody=localizeMessages(msgBodyTag).format(name=', '.join((localizeUpgrade(db.DBLogic.g_instance.upgrades[u]) for u in msgData['upgrades']))), credits=__localizeNegative(msgData['exp']), expPlane=__localizeNegative(msgData['exp_plane']), expFree=__localizeNegative(msgData['exp_free']))
    return imsg
...
def buyUpgrade(imsg):
    ...
    if creds:
        imsg['msgData']['credits'] = __localizeValueNegative(creds)
    elif gold:
        imsg['msgData']['gold'] = __localizeValueNegative(gold)
    return imsg
...
def sellUpgrade(imsg):
    ...
    if creds:
        imsg['msgData']['credits'] = __localizeValuePositive(creds)
    elif gold:
        imsg['msgData']['gold'] = __localizeValuePositive(gold)
    return imsg
...
def sellConsumables(imsg):
    ...
    if spendCredits:
        imsg['msgData']['credits'] = __localizeValuePositive(spendCredits)
    return imsg
...
def buyEquipment(imsg):
    ...
    if spendCredits:
        imsg['msgData']['credits'] = __localizeValueNegative(spendCredits)
    if spendTickets:
        imsg['msgData']['tickets'] = __localizeNegative(spendTickets)
    return imsg
...
def detachEquipment(imsg):
    ...
    if spendGold:
        imsg['msgData']['gold'] = __localizeValueNegative(spendGold)
    return imsg
...
def buySlot(imsg):
    ...
    if spendGold:
        imsg['msgData']['gold'] = __localizeValueNegative(spendGold)
    if spendTickets:
        imsg['msgData']['tickets'] = __localizeValueNegative(spendTickets)
...
def tokenExchanged(imsg):
    ...
    if gold > 0:
        imsg['msgData']['gold'] = __localizeNegative(gold)
    return imsg
...
def __localizeValuePositive(amount):
    credits_ = abs(int(amount))
    return credits_


def __localizeValueNegative(amount):
    credits_ = -abs(int(amount))
    return credits_


def __localizePositive(amount):
    amount_ = abs(int(amount))
    return separateThousandths(amount_)


def __localizeNegative(amount):
    amount_ = -abs(int(amount))
    return separateThousandths(amount_)
...
def goldToTicketExchange(imsg):
    ...
    imsg['msgData']['tickets'] = '{0}{1}'.format('-' if tickets < 0 else '+', __localizePositive(tickets))
    imsg['msgHeader'] = localizeMessages('LOBBY_HEADER_BOUGHT')
    return imsg
...
def warActionWarBegin(imsg):
    ...
    imsg['msgData'] = dict(fraction=imsg['msgData'], msgBody=localizeMessages('SYSTEM_MESSAGE_ALBION_EVENT_BEGAN'))
    return imsg


def warActionWarEnd(imsg):
    LOG_DEBUG('warActionWarBegin', imsg)
    imsg['msgData'] = dict(fraction=imsg['msgData'], msgBody=localizeMessages('SYSTEM_MESSAGE_ALBION_EVENT_END'))
    return imsg
...
def buyPack(imsg):
    ...
    if spendGold:
        imsg['msgData']['gold'] = __localizeValueNegative(spendGold)
    if sendTickets:
        imsg['msgData']['tickets'] = __localizeNegative(sendTickets)
...
def handleWithOutTransform(imsg):
    return imsg
...
def handle(imsg):
    try:
        ...
        getLogger().exception('Wrong message: %s', imsg)
...
def handleMessageGroup(imsg):
    grpList = wgPickle.loads(wgPickle.FromServerToClient, imsg['msgData']['grpList'])
    grpMap = {}
