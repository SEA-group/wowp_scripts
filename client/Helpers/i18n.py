...
def localizeTimeIntervalHM(seconds):
    """
    Return localized string containing hours and minutes of interval.
    Note: if rest seconds >= 30, then rounds up to 1 minute
    @type seconds: int
    @rtype: str
    """
    ...
    if rest >= 30:
        minutes += 1
    return '%s %s %s %s' % (int(hours),
     localizeLobby('COUNTER_HOURS'),
     int(minutes),
     localizeLobby('COUNTER_MINUTES'))


def localizeTimeIntervalMS(seconds):
    """
    Return localized string containing minutes and seconds of interval.
    @type seconds: int
    @rtype: str
    """
    minutes, seconds = divmod(seconds, 60)
    return '%s %s %s %s' % (int(minutes),
     localizeLobby('COUNTER_MINUTES'),
     int(seconds),
     localizeLobby('COUNTER_SECOND'))
...
def separateThousandths(value):
    intValue = int(value)
    separatedValue = '{:,}'.format(abs(intValue)).replace(',', ' ')
    if intValue < 0:
        separatedValue = '- ' + separatedValue
    return separatedValue


def localizeCommonHudLobby(label):
    return convert(makeString('#common_hud_lobby:' + label.upper()))
