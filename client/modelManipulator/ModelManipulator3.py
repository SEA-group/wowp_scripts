...
HIDE_ALL_MODELS_CONDITION_KEY = 'hide_all_models'
HIDE_ALL_MODELS_CONDITION = [(False, HIDE_ALL_MODELS_CONDITION_KEY)]
HIDE_ALL_MODELS_EXCEPTION = 'plane.model'
...
class ObjectDataReader():
    ...
    @staticmethod
    def prepareParts(context, partTypesDb, partByNames, linkedModels):
        ...
        for partDb in modelParts:
            ...
            if partDb.partId in context.partTypes:
                partTypesDb[partDb.partId] = partDb.getPartType(context.partTypes[partDb.partId])
            else:
                partTypesDb[partDb.partId] = partDb.getFirstPartType()
    ...
    def partsRead(builder, context):
        ...
        linkedModels = dict()
        shellModels = dict()
        ObjectDataReader.prepareParts(context, partTypesDb, partByNames, linkedModels)
        ...
        if context.fullLoading:
            for flamePath, flameParticle, uniqueId, shellPath, shellParticle in context.gunsData:
                path = ObjectDataReader._resolvePath(flamePath, '', partByNames)
                debug_utils.LOG_DEBUG('register flame particle', path, uniqueId)
        ...
        for partDb in modelParts:
            ...
            if partType.fireMountPoint != '' and context.fullLoading:
                ...
                builder.addEffect(ObjectDataReader._resolvePath(partType.fireMountPoint, path, partByNames), [[(1, ObjectDataReader.partFireId(partDb.partId))]], effectName, False)
            ...
            for (itemPath, modelName, animName), dataList in models.iteritems():
                ...
                conditionForModel = [condition, HIDE_ALL_MODELS_CONDITION] if modelName[-len(HIDE_ALL_MODELS_EXCEPTION):] != HIDE_ALL_MODELS_EXCEPTION else [condition]
                ...
                if controllerName == 'TurretController' and turretSettings and turretSettings.visualSettings:
                    ...
                    if not turretSettings.isScenarioAnimator:
                        builder.addTurret(itemPath, conditionForModel, gunnerId, modelName, turretSettings, partDb.name)
                    else:
                        for triggerName in partTriggers:
                            builder.addTurret(itemPath, conditionForModel, triggerName, modelName, turretSettings, partDb.name)
                    ...
                    gunnerId = partDb.partId
                    settings = planeVisualSettings.gunnerAnimatorSettings.get(partDb.partId)
                    if settings is not None:
                        builder.addGunnerHeadModel(itemPath, conditionForModel, gunnerId, modelName, partDb.name, settings, context.entityId)
                else:
                    modelId = builder.addModel(itemPath, conditionForModel, modelName, animName, partDb.name)
                    upgrade = partTypesDb[partDb.partId]
                    if hasattr(upgrade, 'gunPartName'):
                        builder.addRelation(modelId, upgrade.gunPartName)
                    if controllerName:
                        controller = builder.addController(itemPath, controllerName)
            ...
            for itemPath, models in propellers.iteritems():
                ...
                if 'PropellorController' in models:
                    rotorModel, meshModel, normalTextureModel, forsageModel, direction = (models['PropellorController'],
                     ...
                     True)
                if context.fullLoading:
                    builder.addPropeller(itemPath, [HIDE_ALL_MODELS_CONDITION], [[(1, ObjectDataReader.partStateId(partDb.partId, 3))]], rotorModel, meshModel, normalTextureModel, forsageModel, direction)
                else:
                    builder.addHangarPropeller(itemPath, [HIDE_ALL_MODELS_CONDITION], rotorModel, meshModel, direction)

            for model, mountPoint, condition in linkedModels[partDb.partId]:
                condition += [HIDE_ALL_MODELS_CONDITION]
...
class ModelManipulator3(object):
    ...
    def getIKSystemsWithPart(self):
        return self.context.ikSystemByPart
    ...
    def setState(self, avatarState, transitionActions):
        ...
        if not self.__context.isBaseObject:
            if avatarState & (EntityHelpers.EntityStates.GAME_CONTROLLED | EntityHelpers.EntityStates.WAIT_START | EntityHelpers.EntityStates.PRE_START_INTRO):
                ...
                self.setEffectVisible('LOFT_LARGE', False)
                self.setEffectVisible('SMOKE', False)
            ...
            if avatarState & EntityHelpers.EntityStates.DESTROYED:
                if self.__context.isAircraft:
                    self.setCondition(HIDE_ALL_MODELS_CONDITION_KEY, True)
            ...
            if avatarState & EntityHelpers.EntityStates.DEAD:
                ...
                self.setEffectVisible('SMOKE', False)
                self.setEffectVisible('ATTACKANGLE', False)
                self.setEffectVisible('BOMBERS_TRAIL_INIT', False)
    ...
    def setFlapsAxisValue(self, value):
        """control Flaps axis value change - from PLayerAvatar"""
        if value > 0:
            self.__flapsOppened[consts.FLAPS_AXIS] = True
            self.setAxisValue(consts.FLAPS_AXIS, value)
        else:
            self.__flapsOppened[consts.FLAPS_AXIS] = False
            self.setAxisValue(consts.FLAPS_AXIS, value)

    def setAxisValue(self, axis, value):
        """on control axis value change - for animation"""
        if self.__animatorsController:
            if axis == consts.FLAPS_AXIS:
                if value <= 0 and any(self.__flapsOppened.values()):
                    return
            self.__checkAxisEffects(axis, value)
            self.__animatorsController.setAxisValue(axis, value)

    def __createAxisEffectsAssociates(self):
        self.__axisEffectsAssociates = {consts.FORCE_AXIS: self.__checkForceAxisEffect}
        if not clientConsts.FLAPS_EFFECTS_ONLY_ON_ENGINE_OFF:
            self.__axisEffectsAssociates[consts.FLAPS_AXIS] = self.__checkFlapsAxisEffect

    def __checkAxisEffects(self, axis, value):
        self.__axisEffectsAssociates.get(axis, lambda x: None)(value)

    def __checkForceAxisEffect(self, value):
        self.setEffectVisible('ENGINE', value > -1)
        self.setEffectVisible('JETENGINE', value >= -1)
        if self.__context.isPlayer and value <= 0:
            if value < 0:
                self.__flapsOppened[consts.FORCE_AXIS] = True
            else:
                self.__flapsOppened[consts.FORCE_AXIS] = False
            self.setAxisValue(consts.FLAPS_AXIS, max(0, -value))
            if clientConsts.FLAPS_EFFECTS_ONLY_ON_ENGINE_OFF:
                self.__checkFlapsAxisEffect(max(0, -value))

    def triggerAnimation(self, trigger, value):
        if self.__animatorsController:
            self.__animatorsController.setTriggerValue(trigger, value)

    def __checkFlapsAxisEffect(self, value):
        self.setEffectVisible('BRAKE', 0 < value <= 1)

    def updateForsageEffects(self, value = None, sniperEnabled = None):
        """on forsage event"""
        if value is not None:
            self.__isForsageOn = value
        self.setEffectVisible('FORSAGE', self.__isForsageOn)
        self.setEffectVisible('JETFORSAGE', self.__isForsageOn)
        if clientConsts.SNOWBALLS_MOD.canPlayBengalFire(self.consumablesEffects):
            self.setEffectVisible(clientConsts.SNOWBALLS_MOD.BengalFireEffect, self.__isForsageOn)
        else:
            self.setEffectVisible('LOFT_LARGE', self.__isForsageOn)
        return

    def updateBombersTrailEffect(self, value):
        if not self.__context.isDestroyed:
            self.setEffectVisible('BOMBERS_TRAIL_INIT', value)
    ...
    def setVisible(self, value):
        LOG_DEBUG('Set model visible:', value)
    ...
    def destroy(self):
        ...
        self.__axisEffectsAssociates.clear()
        CompoundSystem.clearCompound(self.compoundID)
        return
    ...
    def updateConsumablesEffects(self, consumables):
        ...
        for consumableData in consumables:
            ...
            if key != -1 and (key in clientConsts.SNOWBALLS_MOD.TRIGGERS.values() or key == clientConsts.SNOWBALLS_MOD.BengalFireConsumableID):
    ...
    def getTurretController(self):
        return self.__animatorsController.getController('TurretAnimator')

    def getHeadController(self):
        return self.__animatorsController.getController('TurretAnimator')

    def updateSpeedwiseEffects(self, avatarEntity):
        if EntityHelpers.isPlayerAvatar(avatarEntity):
            currentSpeed = avatarEntity.getSpeed()
            currEngineId = avatarEntity.logicalParts[consts.LOGICAL_PART.ENGINE]
            curHullID = avatarEntity.logicalParts[consts.LOGICAL_PART.HULL]
            engineMaxSpeed = -1
            if EntityHelpers.EntityStates.inState(avatarEntity, EntityHelpers.EntityStates.GAME):
                engineMaxSpeed = avatarEntity.settings.airplane.flightModel.engine[currEngineId].maxSpeed
            diveSpeed = avatarEntity.settings.airplane.flightModel.hull[curHullID].diveSpeed
            effectStartSpeedCoef = avatarEntity.settings.airplane.visualSettings.speedwiseEngineEffect
            self.__updateSpeedwiseEngineEffects(currentSpeed, engineMaxSpeed, effectStartSpeedCoef)
            self.__updateSpeedwiseWingEffects(currentSpeed, diveSpeed)
            self.__animatorsController.updatePlaneSpeed(currentSpeed, engineMaxSpeed, diveSpeed)

    def __updateSpeedwiseWingEffects(self, currentSpeed, diveSpeed):
        curSpeedCoef = currentSpeed / diveSpeed
        self.setEffectVisible('SPEED_OVERLOAD', curSpeedCoef >= clientConsts.SPEED_OVERLOAD_MIN / 100.0)

    def __updateSpeedwiseEngineEffects(self, currentSpeed, maxSpeed, effectStartSpeedCoef):
        curSpeedCoef = currentSpeed / maxSpeed
        curSpeedEffectID = -1
        if curSpeedCoef >= effectStartSpeedCoef:
            if self.__isForsageOn:
                curSpeedEffectID = 3
            else:
                curSpeedEffectID = 2
        else:
    ...
    def reloadPartControllersShakeSettings(self):
        self.__animatorsController.reloadControllerSettings()