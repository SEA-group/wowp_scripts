...
class DrawObjectBase(object):
    ...
    def onVisibleChange(self, visible):
        ...
        if not CompoundSystem.isHandleValid(self._context.cidProxy.handle):
            if self._context.cidProxy.handle != INVALID_ID:
                debug_utils.LOG_ERROR('Handle is not valid for object')
            return
...
class TrailEffectObject(SoundObject):

    def _onVisibleChange(self, visible):
        SoundObject._onVisibleChange(self, visible)
        if consts.IS_EDITOR or visible:
            CompoundSystem.changeParticleVisible(self._context.cidProxy.handle, self.id, visible)
        elif not visible:
            CompoundSystem.sliceTrailParticle(self._context.cidProxy.handle, self.id)
        CompoundSystem.changeParticleActive(self._context.cidProxy.handle, self.id, visible)
...
class ObjectsBuilder:
    ...
    def __init__(self, animationController, boolCombiner, eventSystem, context, onLoadedCallback):
        ...
        self.__drawObjects = []
        self.__relationGunnerWithTurret = dict()
    ...
    def addGunnerHeadModel(self, path, visibleCondition, gunnerId, model, partName, settings, entityId):
        if not model:
            return
        ...
        self.__registerDrawObj(modelObject)
        animator = self.__animationController.getController('TurretAnimator')
        tracker = animator.createHeadTracker(gunnerId, nodeId, modelObject.id, settings, entityId)
        if tracker:
    ...
    def addTurret(self, path, visibleCondition, gunnerId, model, turretSettings, partName):
        if not model:
            ...
            self.__registerDrawObj(modelObject)
            animator = self.__animationController.getController('TurretAnimator')
            tracker = animator.createTurretTracker(gunnerId, nodeId, turretSettings, gunIds, path)
    ...
    def addParticle(self, path, playId, particleFileOrFiles):
        ...
        for particleFile in particlesList:
            debug_utils.LOG_DEBUG('addParticleToList', node.id, particleFile, path)
            self.addParticleToList(node.id, False, particleFile, particleFile)
    ...
    def __selectAndPlayParticle(self, particlesList, firstObjID, delay):
        ...
        CompoundSystem.changeParticleVisible(self.__context.cidProxy.handle, idObj, 1)
    ...
    def addEffect(self, path, visibleCondition, name, timed, delayRange = None):
        ...
        if effectDB:
            ...
            if effectType == 'Loft' or effectType == 'Jet':
            ...
            elif effectType == 'TimedParticle' or effectType == 'LoopParticle' or effectType == 'TrailParticle':
                ...
                if resourceName:
                    ...
                    elif effectType == 'TrailParticle':
                        effectObject = TrailEffectObject(self.__context, objId, sound)
                    else:
    ...
    def addRelation(self, gunnerModelId, turretName):
        self.__relationGunnerWithTurret.setdefault(turretName, []).append(gunnerModelId)
