...
class ConnectionManager(object):
    ...
    def connect_with_token1(self, url, token1, spaID, publicKeyPath = None, nickName = None):
        if url is not None:
            LOG_DEBUG('connect_with_token1 || url: %s; token1: %s; spaID: %s' % (url, token1, spaID))
            ...
            loginInfo = LoginInfo()
            dct = {'login': '',
             'auth_method': AUTH_METHODS.TOKEN1,
    ...
    def connect_with_token2(self, url, token2, publicKeyPath = None, nickName = None):
        if url is not None:
            LOG_DEBUG('connect_with_token2 || url: %s; token2: %s' % (url, token2))
            ...
            loginInfo = LoginInfo()
            dct = {'login': '',
             'auth_method': AUTH_METHODS.TOKEN2,
             'session': md5hex(getHardwareID())}
            loginInfo.inactivityTimeout = CLIENT_INACTIVITY_TIMEOUT
    ...
    def connect(self, url, login, password, publicKeyPath = None, areaID = None, nickName = None, token2 = '', rememberPassword = False, authMethod = None, token1 = '', spaID = None):
        self.disconnect()
        LOG_DEBUG('url: %s; login: %s; pass: %s; name: %s; areaID: %s; authMethod: %s; token1: %s; token2: %s; spaID: %s' % (url,
         login,
         password,
         nickName,
         areaID,
